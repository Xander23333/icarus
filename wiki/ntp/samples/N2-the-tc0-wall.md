...
### 2026-05-30 update：把 C-FORM-7/8/8b 三角钉回 §10 同一根 M1 负反馈链

写完 §八 主段三天之后（2026-05-29 14:01Z [`formal_limits.md` M1 reviewer pass commit `bd91f2a`](../topics/formal_limits.md) + 同日 16:35Z [`../survey/ntp_survey.md` §10 中段元-审计 commit `64438d4`](../survey/ntp_survey.md)），formal 域的三条 sub-candidate / corollary (C-FORM-7 MoR 同 FLOPs 比较 / C-FORM-8 SSM × Loop / C-FORM-8b SSM × CoT) 与本章 §七 / §八 的关系被 §10 元-审计正式收紧。按 N4 §八 / N7 §八 / N3 §七 / N1 §六 / N5 §八 / N6 §八 已建立的 sample↔§10 双向同步纪律 (sample 节数止于 §八，新增 update 以 `### YYYY-MM-DD update` 子节落在 §八 内不开 §九)，本子节做 N2 这一章的第七次同步——只贴归属、不新增数字。

第一件事，**§八 那张 2×2 小地图与 M1 集合 $\{m_k\}$ 的对应**。§10 中段元-审计把 frontier-disclosure 缺位作为 mech-relevant metric 系统性缺位 (M1) 锁定 $k=1..8$ 八条 $m_k$，formal 域 reviewer pass 把三条 sub-candidate 显式归属如下：C-FORM-8 = $m_4$ (state-tracking $S_5$) × $m_6$ (video-backbone 披露选择函数) *双投影*，$m_4$ 侧 evidence 强 (Merrill *Illusion of State* [arxiv:2404.08819](https://arxiv.org/abs/2404.08819) + Giannou 2301.13196 两行同构)，$m_6$ 侧 evidence 弱 (无 frontier 部署 $T_{\max}=\Omega(\log n)$ 通路故无对照沉默直接观测)；C-FORM-8b = $m_4$ × $m_5$-同型 (mech + 学界分工)，与 C-FORM-8 构成 *内化 vs 外化 scratchpad* sibling pair，但社会学归因不同——C-FORM-8 的沉默归 *披露选择函数* (商业敏感)，C-FORM-8b 的沉默归 *学界分工* (SSM 派关心吞吐 / formal 派关心 attention)；C-FORM-7 = $m_9$ 候选 (架构-调度旋钮，与 grounding C-GROUND-PROTO 同节奏，2026-06-10 12 天冻结到期后再审 (i)(ii)(iii) 三阈值)。三条 sub-candidate 分别落到三种不同 M1 邻居关系，阻止 §10 (4) 列表被误算 *独立 $m_k$* 推动膨胀——这一归属化把 §八 末段 "*frontier 默认部署 CoT 却从不公开 backbone × scratchpad controlled comparison*" 的社会学缺位描述从 N2 单章修辞 升级到 §10 跨域共享元命题的 formal 域投影。

第二件事，**§八 那条 "*1B Mamba + 1B Llama 各一档跑 length-100/200/500 即可判，但无人接手*" 的 learnability 缝**，在 reviewer pass 之后获得 *跨域共担实验* 状态：formal_limits reviewer 把 controlled comparison 三轴 (α) 同 token 量训练 + 同 CoT 长度上限 + 同 prompt 模板 / (β) 评估 out-of-train-distribution length / (γ) 报告 length/accuracy 曲线而非单点 显式钉死——这一三轴与 reasoning C-REAS-6 三档 selection signal 拟合协议 / grounding C-GROUND-PROTO 三轨同一 forward pass 同步采集 ρ 同型 (*<2 GPU-week + 发表回报为负*)，构成 §10 第八种几何加密方式 *时间侧对偶* 在 formal 域的第三个落点 (前两为 OP-SCALE 半年核查 / C-CAUSAL-DISCLOSE 三个 <1 GPU-week 实验)。这意味着 §八 那条 learnability 缝在 sample 层 *不应再被反复挂账* — 它已在 §10 元-审计层有可机械判定的 falsifier (任一团队在三轴协议下跑 1B Mamba vs 1B Llama × length-100/200/500 即清零)。

第三件事，**12 天冻结协议 (2026-05-30 → 2026-06-10) 对 N2 章节的约束**。§七 末段 "如果 C-FORM-4 到 2027-12 仍无形式 lower bound...本章判断会从 *墙在 expressivity* 退到 *墙在 learnability*" 这条挂账，在冻结协议下不应在 sample 层主动展开——M1 元命题已经把 "mech 命题加密 ≠ mech 信号增强" 这条元方法论结构在 §10 内部用 *固定 8 条 $\{m_k\}$ + 12 天冻结 + 候选 $m_9$ 暂不申请扩入* 三层硬约束实施了，N2 §七 那条挂账若同步在本章扩段会与 §10 形成 *双重命名同义反复*。本子节的 *实操承诺* 是：N2 §七 挂账暂存到 2026-06-10 冻结到期后再决定 — 若届时 C-FORM-7 升入 $m_9$，N2 §八 须在 *本子节* 内打 "2026-06-10 update" 二级子标题增量更新，不开 §九；若届时冻结协议续期且 $\{m_k\}$ 仍锁 8 条，则本章节数永久止于 §八。

**诚实判断**：本子节是 N2 全章 *首次主动放弃一个挂账* 而非新增一个判断——与 N7 §八 / N4 §八 / N5 §八 "主动收缩一档" 同型，是 sample 系列在 12 天冻结协议下的第七次方向收紧而非扩张。最大方法学风险与 N4 §八 元判断一致：把 M1 元命题当万能容器，使 §八 2×2 小地图的具体性被稀释到 "反正都是 frontier-disclosure 在作怪"。规避协议三条 (a) M1 命名 *仅出现在本子节*，§八 主段 2×2 表 + Merrill 2404.08819 + Giannou 2301.13196 + Brösamle-Eckstein 2605.18079 [unverified ID] 三条 evidence 锚不被回写、不被打标签；(b) sample 层 N2 不在 §九/§十 开新通道，未来更新一律走本子节 in-place 增量；(c) §七 / §八 的 *可证伪坐标* 不被 M1 替换——仍以 "*1B Mamba + 1B Llama × length-100/200/500 三轴 controlled comparison*" 与 "*任一 frontier lab 公开 backbone × scratchpad 内化 vs 外化 head-to-head 数字*" 两件具体事件作 N2 全章总验收清单。两件事在 2026-05-30 这个时点均 [unknown]，Brösamle-Eckstein 2605.18079 / Schuurmans 2024 论文 ID 仍标 [unverified]，未编造数字 / 日期 / 标题。

*相关 topic：[formal_limits](../topics/formal_limits.md)（C-FORM-7/8/8b 三角 M1 归属与 falsifier 三处歧义反向收紧）、[reasoning](../topics/reasoning.md)（C-REAS-6 三档 selection signal 拟合协议 = M1 $m_5$，与本子节 1B Mamba vs 1B Llama controlled comparison 同型）、[grounding](../topics/grounding.md)（C-GROUND-PROTO 三轨同一 forward pass 同步采集 ρ = 候选 $m_9$，方法学层同型）；同时与 [`../survey/ntp_survey.md` §10 中段元-审计 (commit `64438d4`)](../survey/ntp_survey.md) 双向锚定 M1 与 12 天冻结协议。*

### 2026-05-30 update：Chess-WM 2605.30100 — §六三道暗门旁的「第四道暗门候选」与 §八 2×2 backbone 一维的实证骨架

2026-05-30 daily harvest 落了 Walker & Lyons *Chess-World-Model* ([arxiv:2605.30100](https://arxiv.org/abs/2605.30100); paper note `wiki/ntp/papers/paper_notes/2026-05-30-2605.30100-chess-world-model-state-tracking.md`)：10M 真实国际象棋对局 + matched-protocol 对照 Causal Transformer / block-diagonal SLiCE / Mamba-3 / Gated DeltaNet (带 negative eigenvalues)，3M–8M 参数规模 recurrent / SSM-with-neg-eig 在 *exact state tracking* 上显著胜 attention-only Transformer，real-game held-out 在 ≥18M 参数饱和，但 uniform-legal OOD split 持续不饱和。这条结果在 [`../topics/formal_limits.md` §C-FORM-6 实证 anchor 段](../topics/formal_limits.md)（2026-05-30 daily commit `ab19e5d` + [`../topics/world_model.md` §(b) Chess-WM 横向落点](../topics/world_model.md)）已分别落到 formal × world_model 两侧；paper note Reviewer 段把它点名为 \"*把 N2 三暗门扩成四暗门候选*\" 的 *sample 层未落* 子弹。本 in-place 子节做该落地，严守 2026-05-30 → 2026-06-10 12 天增条冻结协议白名单 *evidence-base refinement + 互引补充* 两类操作，**不** 新增 sub-candidate / corollary / 不动 §六三道暗门修订表 / 不动 §八 2×2 小地图四格命名 / 不动 §七赔率 / 不动本章 §三 Merrill–Sabharwal 主表达力上界。

**先点出 *候选* 第四暗门的精确形状**。§六三道门分别破的是 (i) 中间 step 离散 / (ii) 单次 forward / (iii) 输入表征中性。Chess-WM 的实证胜场指向 *第四类假设*：**Merrill–Sabharwal 上界证明里隐含 \"backbone 单步类的 transition matrix 表达力对 worst-case 阶不重要\" 这一前提**——结论确实是 attention-only NTP 与 selective-SSM 同属 log-precision $\\mathsf{TC}^0$（Merrill *Illusion of State* [arxiv:2404.08819](https://arxiv.org/abs/2404.08819) 已钉死同上界类），但在 *固定参数规模 × matched protocol × 真实大规模 domain* 的 learnability 切面上，*input-dependent + negative-eigenvalue* 的 transition matrix 选择给 SSM 一条 attention-only 没有的可学习胜场。这道门 *不挪 worst-case 表达力上界* (与 §八 \"四格塌缩成同一条曲线\" 一致)，*只* 挪 learnability 维度上每个 backbone 单步类所能 *实际学到* 的状态压缩深度——也就是把 §八末段 *learnability 缝* 从 \"无人接手\" 的 [unknown] 升级到 \"已被一个 closed-world 玩具的 10M-scale 实证占了一格\" 的 *方向性* anchor。这条门 *不写入 §六修订表* 也 *不开 N2 §九*——它是冻结协议下的 *第四暗门候选* (candidate)，2026-06-10 复查时若 C-FORM-6 或 C-FORM-8 在 topic 侧升级强度才考虑写入正文 §六表 (与 §七 C-FORM-4 挂账 / N7 §八 stealth-CPT 命名 同型节奏)。

**与 §八 2×2 小地图的对位**。§八已把 *(backbone, scratchpad)* 拆出四格 (attention, loop) / (attention, CoT) / (SSM, loop) / (SSM, CoT)。Chess-WM 实证恰好把 *backbone 一维* 的两端 (attention vs SSM) 在 *zero scratchpad* 设定下做了 head-to-head 测量——即把整张 2×2 投影到 *scratchpad = none* 这一行，得到 *backbone-only learnability* 的第一条 10M-scale + matched-protocol 数字。这条数字对 §八 主段两个断言各起一次作用：(a) 对 \"*worst-case 跨四格塌缩成同一条曲线*\" 这条 *不动*——Chess-WM 没有测 CoT 或 loop 任一阶 (paper 设定即 single-pass)，所以四格塌缩命题在 worst-case 维度仍未被触动；(b) 对 \"*learnability 差异才是真正未测的缝*\" 这条 *部分填上*——在 zero-scratchpad 切面上 SSM 胜场显著，**但 OOD random-uniform 不饱和数据点同时说明 \"胜场\" 仅在 in-distribution held-out 成立，shortcut-learning 仍是 attention-only NTP 的 state-tracking 真实失败主因**，与 §七第一条 C-FORM-4 \"NTP-learnability gap\" 内圈墙同源 (Chess-WM OOD 不饱和 = shortcut-learning 在 state-tracking 子带的 *跨 backbone* 共同上界)。这一双向修订 *不* 改变 §八 1B Mamba × 1B Llama × length-100/200/500 三轴 controlled comparison 仍是总验收清单 (Chess-WM 是 chess domain 而非 *length 泛化* 维度，两条实验互补不替代)。

**三处强度限定** (与 N7 §九 第三支柱 \"reviewer 收紧\" / N3 §七 reviewer 加注 同型节奏)：(α) Chess deterministic + 完全可观察 state，与 §核心问题 *partial observability + stochasticity + causality* 正交——本条 anchor *不* 升 SSM 路线到 \"更接近 world model\" 的强结论 (与 [`../topics/world_model.md` §(b)](../topics/world_model.md) 同步限定一致)；(β) 18M 参数饱和点未必外推到 frontier 规模 NLP/video，把 closed-world 玩具的 backbone-learnability gap 外推到 reasoning model 是危险动作 — paper-note Reviewer 段已自我警告过一次，本节复述；(γ) Chess-WM 与 Merrill 2404.08819 共享同一 SSM × expressive-transition 论证骨架，*不* 算两条独立证据通道，与 [`../topics/online_learning.md` §216 行 DCV 第三支柱独立性核查](../topics/online_learning.md) 单一证据 × 两投影 同型——本节 anchor 算 C-FORM-6 evidence-base 第一条 *跨 backbone matched-protocol 实证*，不算 C-FORM-1 (TC⁰ wall) 的独立 sibling，与 [`../topics/formal_limits.md` §C-FORM-6 实证 anchor 段](../topics/formal_limits.md) 处理一致。

**互引** (单向)：本子节单向引 [`../topics/formal_limits.md` §C-FORM-6 实证 anchor](../topics/formal_limits.md) (commit `ab19e5d`) 与 [`../topics/world_model.md` §(b) Chess-WM 横向落点](../topics/world_model.md) (commit `0a17d6b`)，*不* 反向要求两 topic 回贴 N2——sample 是 *已落 topic 决策的叙事层投影*，方向与 N6 §九 单向引 C-WM-6 / C-WM-7 同型，与 §10 (c) 冻结协议 *互引补充* 白名单一致。

**判断**：本子节是 N2 进入 sample↔daily harvest 双向同步纪律的第二次落地 (第一次为 §七 Brösamle-Eckstein 2605.18079 [unverified ID])，与 N3 §七 (DCV) / N4 §八 (YoCausal) / N6 §九 (M1 编号层) / N7 §九 (stealth-CPT 第三支柱对偶) 共同构成 2026-05-30 daily harvest 五颗子弹的 sample 层第五处落地——Chess-WM 是这五处中 *唯一 backbone 维度* 的 anchor (其余四条均在 mech 命题维度)。**不改 §三 / §六 / §八 任一既有判决**，只把 §八末段 *learnability 缝* 从 [unknown] 升到 *已有一格方向性证据*；**不改 §七赔率**——本章无显式赔率列表，相当默认不动；**不申请 §六修订表新行**——第四道暗门状态保持 *候选* 直到 2026-06-10 冻结到期。最大方法学风险与 §八 末段 \"M1 万能容器\" 警告同向：把 Chess-WM 的 closed-world 胜场过度泛化到 frontier reasoning model 失败模式。规避做法：本子节所有 anchor 段落严格用 *closed-world / matched-protocol / 18M 饱和点* 三处限定词，未出现 \"frontier\" / \"reasoning model\" / \"generalize\" 等扩张性术语；OOD random-uniform 不饱和被显式标注为 *attention-only NTP shortcut-learning 共同上界* 而非 *SSM 优越性证据*。两件 2026-06-10 复查触发条件 (i) C-FORM-6 在 topic 侧升级到 medium-strong / (ii) 任一 frontier lab 公开 backbone × state-tracking head-to-head 数字 — 均 [unknown]，2605.30100 paper ID 已 verified via paper_notes，未编造数字 / 日期 / 标题 / 作者。

### 2026-05-31 update：reviewer 加注 type 几何直标——N2 = type II archetype 的 sample 侧主场投影

写完 Chess-WM 子节一天之后，[`../survey/timeline.md` meta-row *type-geometry 同步链 7 落点闭合登记* (commit `29107ec`, 2026-05-31 08:30Z)](../survey/timeline.md) 把 type I/II/III 落点几何同步链 *形式闭合* 于 [`../topics/world_model.md` reviewer 加注 (commit `f261f3e`, 2026-05-31 04:05Z)](../topics/world_model.md)，并在第 4 条方法学边界条款里规定：**链终点收束后，剩余 ≥4 未覆盖 topic / sample 直接 type 标注、不追加新同步链**。topic 侧此条已在 [`embodiment.md`](../topics/embodiment.md) (`dbf27f3`) / [`grounding.md`](../topics/grounding.md) (`3901303`) / [`online_learning.md`](../topics/online_learning.md) (`5fce409`) / [`scaling_limits.md`](../topics/scaling_limits.md) (`f190f2b`) 四处兑现完毕；sample 侧首两处为 [`N5-embodiment-and-vla-bet.md` §九末 (commit `491ea4e`)](N5-embodiment-and-vla-bet.md) 与 [`N7-why-llm-cannot-continually-learn.md` §十 (commit `6c5a216`)](N7-why-llm-cannot-continually-learn.md)。本 in-place 子节做 N2 章——sample 侧第三处也是 *type II archetype 主场对位* 的兑现，严守 12 天冻结协议 (2026-05-30 → 2026-06-10) 白名单 *互引补充 + 升降级历史登记 + 措辞收紧* 三类，**不** 新增 sub-candidate / corollary / route-elim / 不动 §三 Merrill–Sabharwal 主表达力上界 / 不动 §六三道暗门修订表 / 不动 §八 2×2 backbone × scratchpad 四格命名 / 不动上一子节 Chess-WM *第四暗门候选* 状态 / 不引入新 arxiv ID。

**(一) KFW 2605.27734 type II 直接落点：N2 = type II archetype 的 sample 侧 *主场* 投影。** taxonomy [`f61f8c4`](../survey/taxonomy.md) 升降级历史已把 *KFW = type II archetype* (theory-side sample-complexity，跨 topic *不共享 target* 故 *independent measurements = 1*) 钉死，[`formal_limits.md` (α') reviewer 段 (commit `1c22b75`)](../topics/formal_limits.md) 把 formal_limits 标为 *type II 主场 topic* (KFW 四 target 之一就是 NTP-learnability 即 §七 C-FORM-4 内圈墙)。N2 章 §三 Merrill–Sabharwal 主线 + §七 第一条 C-FORM-4 挂账 + §八 2×2 *worst-case 四格塌缩* 三处合起来构成 KFW *第五 target* 在 sample 侧的 *叙事层投影*——但按 *跨 topic / 跨 sample 不共享 target* 协议，五 target 的 *independent measurements 仍 = 1* (KFW 单一作品在 N2 章只增 *投影面*，不增 *独立证据条数*)。此一直标对 §七 末段 \"内圈墙先撞\" 与 §八 末段 \"learnability 缝才是真正未测的\" 两条挂账各起一次 *方向不动 + 强度封顶* 作用：方向不动指 internal-wall first 与 learnability gap 两断言保留；强度封顶指 *不能把 KFW 出现在 N2 当成 C-FORM-4 第四独立 anchor*。本子节即把 N2 章正式注册为 KFW *sample 侧主场投影第一处* (与 N5/N6 §九 / N7 §九的 KFW 提及不同——那三处是 *负空间登记* 或 *理论侧旁证*，N2 是 *叙事层主场*)。medium 强度不动。

**(二) DCV 2605.28860 type I 显式不落点：N2 章 *负空间登记* 第一处。** DCV 是 type I archetype (training-side mechanistic, RL-vs-SFT layer-attribution + factual-claim 子域 + RL-preserves-circuit 三层根因)，跨 topic 七正落点 (causality / world_model / grounding / scaling / reasoning / online_learning / N3 §七 + N7 §九)。N2 章主题 *worst-case 表达力上界 + scratchpad 内化/外化* 与 DCV 三层根因 *测量层错位*——formal 域 *worst-case 渐近* 自变量与 DCV *attention head 参数空间几何 × final accuracy* 自变量在测量轴上 *不可线性外推* (与 [`formal_limits.md` (β') 段](../topics/formal_limits.md) DCV formal 域负投影同源)。本子节把 N2 章作 DCV 在 *sample 侧 type I 第二负投影位* 登记 (第一负投影位为 [`N3-three-puzzle-pieces.md` §七 (commit `1f96072`) MiraBench 显式不落地决定](N3-three-puzzle-pieces.md) 的姐妹组——N3 是 MiraBench type III 负空间，N2 是 DCV type I 负空间，两处合起来构成 sample 侧 type I/III 两类负投影 *双 anchor*)。三层根因显式化：(α) layer-attribution 的因变量 = circuit 探针前后差，与 N2 §三/§六的因变量 = formal worst-case 时间/空间复杂度 *不同函数空间*；(β) factual-claim 子域与 N2 *算法可计算性 / state-tracking* 子域 *不重叠*；(γ) RL-preserves-circuit 的训练面动力与 N2 *无关训练动力的 worst-case 论证骨架* 正交。

**(三) MiraBench 2605.29360 type III 显式不落点：N2 章 *负空间二连* 完成 sample 侧三类负空间收编。** MiraBench 是 type III archetype (surface-side evaluation-protocol, VLM-arbitrated executor 9 落点协同回退)，与 N2 章 *形式证明 / 形式表达力上下界 / 复杂度类 inclusion* 维度 *没有同型 confound 通道* (与 [`formal_limits.md` (β') 段](../topics/formal_limits.md) MiraBench formal 域 *负落点* + N3 §七 MiraBench *sample 侧负空间* 同型)。N2 章本子节把 MiraBench 显式登记为 type III *第二 sample 侧负空间* (第一为 N3 §七 `1f96072`)，二者构成 *type III 在 N3/N2 两章双负空间* 几何形态——与 [`N5-embodiment-and-vla-bet.md` §九末 (commit `491ea4e`)](N5-embodiment-and-vla-bet.md) 的 *type I+II 双负空间 + type III 二次折扣* 6:2 形态形成对位 (N2 = 类型 II 单正 + 类型 I/III 双负空间 = 1:2 形态；N5 = 类型 I+II 双负 + 类型 III 二次折扣 = 0:3 形态；N7 = 类型 I 直接 + 类型 II 显式不落 + 类型 III 派生 = 派生 triple coverage)。N2 = sample 侧 *三种最简覆盖形态* (单正双负 / 全负 / 派生 triple) 中 *单正双负* 这一档的首例 anchor。

**(四) N2 在 sample-type-geometry 几何空间里的位置。** sample 侧已落四处 type 几何 (N1 §八 `764ecb8` framework-tier 4 anchor 同 archetype 第一处 / N5 §九末 `491ea4e` 6:2 / N7 §十 `6c5a216` 派生 triple / 本节 N2 = 1:2 单正双负)。四处合起来构成 *sample 侧 type-geometry 4 档形态梯度* (framework / 6:2 / 派生 triple / 1:2 单正双负)，与 topic 侧 (world_model direct triple / scaling 派生 triple / embodiment 单覆盖 / grounding / online_learning / formal type II 主场) 6 处共享 *type 是诊断工具非新候选* 元方法论——四处 sample 形态 *不* 算 4 条独立 *type 几何* 证据 (sample 是 *已落 topic 决策的叙事层投影*, 与 N6 §九 / N7 §十 单向引同型, 与 [`../survey/ntp_survey.md` §10 reviewer 加注末段](../survey/ntp_survey.md) *anchor type 是诊断工具非候选* 声明一致)。

**(五) cross-type 复合 falsifier 协同收紧动作链。** 任两条触发 *N2 §六修订表第四道暗门候选* (Chess-WM 子节登记) 升级到 *显式条目*——这正是 cross-type 是 *收紧非新增* 协议在 N2 章的第一次具体化：触发条件不是 \"再加一条 anchor\"，而是 \"type I (DCV-类 backbone learnability 探针) + type II (KFW-类 sample-complexity 下界) + type III (MiraBench-类 evaluation-协议外部审计) 三类中任两类对 *Chess-WM zero-scratchpad SSM 胜场* 出现 *方向一致的二次实证*\" → 才进入 §六修订表。这条门槛比 \"任一 anchor 出现\" 严格一档，与 [`world_model.md` (ε)](../topics/world_model.md) / [`embodiment.md` (ε)](../topics/embodiment.md) / [`scaling_limits.md` (ε)](../topics/scaling_limits.md) cross-type 复合 falsifier 同型。同时把 §八 末段 *1B Mamba × 1B Llama × length-100/200/500 三轴 controlled comparison* 总验收清单标注为 *cross-type 协同的 type I 子通道* (不动三轴判定阈值，仅条件化测量空间)。

**(六) 三处单向引 + 鉴定**。本子节单向引 [`../survey/timeline.md` meta-row (commit `29107ec`)](../survey/timeline.md) 链终点收束条款第 4 条 + [`../topics/formal_limits.md` (α') type II archetype 主场 (commit `1c22b75`)](../topics/formal_limits.md) + [`../samples/N5-embodiment-and-vla-bet.md` §九末 (commit `491ea4e`)](N5-embodiment-and-vla-bet.md) 与 [`../samples/N7-why-llm-cannot-continually-learn.md` §十 (commit `6c5a216`)](N7-why-llm-cannot-continually-learn.md) 两处 sample 侧前序兑现——四处单向引不形成新链。

**判断**：本子节是 N2 进入 *type-geometry 直接标注* 协议的 sample 侧第三处 (N5 第一 / N7 第二 / N2 第三)，也是 sample 系列 *方向收紧 + 强度封顶* 方法学纪律的第三次落地 (前两次 N7 §十 / N5 §九末)。N2 在 type 几何中位置的 *主场性* 与 *负空间双登记* 同时存在 (type II 主场 + type I/III 双负) — 这是 sample 侧首例 *主场 + 双负空间* 共生形态，与 N1 §八 framework-tier 4 anchor / N5 6:2 / N7 派生 triple 三种已有形态 *互补不重叠*。**不改 §三 / §六 / §八 任一既有判决 / §七 内圈墙挂账方向 / 上一子节 Chess-WM 第四暗门候选状态**，只把 N2 章在 sample-type-geometry 4 档梯度中 *正式落位*。规避协议 (a) type I/II/III 命名 *仅出现在本子节*，§三 Merrill–Sabharwal / §六三道暗门 / §八 2×2 / Chess-WM 第四暗门 *四处既有 anchor 段不被回写标签*；(b) §七 内圈墙 *可证伪坐标* 不被 type 命名替换——仍以 *Allen-Zhu Physics-of-LM 把崩塌点定位到具体 attention head* 与 *1B Mamba × 1B Llama × length-100/200/500 三轴 controlled comparison* 两件具体事件作 N2 全章总验收清单；(c) 12 天冻结协议到期 (2026-06-10) 后若 *Chess-WM 第四暗门候选* 升入 §六修订表，须在 *本子节* 内打 \"2026-06-10 update\" 二级标题增量更新，不开 §九。13 commit hash (`29107ec` / `f261f3e` / `1c22b75` / `764ecb8` / `ef1dfe2` / `f61f8c4` / `491ea4e` / `6c5a216` / `dbf27f3` / `3901303` / `f190f2b` / `4707f45` / `3cf73cd`) 经 git cat-file -t 验证为 commit 对象，3 arxiv ID (KFW 2605.27734 / DCV 2605.28860 / MiraBench 2605.29360) 经 paper_notes 交叉验证，无 ID / 数字 / 日期 / 作者 fabricated。Rotation guard：前 tick (`6c5a216`) 触 N7，本 tick 触 N2 不同文件；N2 上次 edit ~22h 前 (Chess-WM 子节)，非连续两 tick。

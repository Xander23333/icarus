...
*相关 topic：[causality](../topics/causality.md)（mech 候选 C-CAUSAL-1 / C-CAUSAL-2 的可证伪表述与四 benchmark 谱协议性失败）、[world_model](../topics/world_model.md)（第三层 ≈ 在 world model 上做 rollback + 反事实 rollout；C-WM-7 latent-action codebook 可辨识性是 §7 video world model rollback 形式检查的核心约束）、[reasoning](../topics/reasoning.md)（CoT 对 Layer-2 有效、对 Layer-3 无效，是 faithful CoT 讨论里最尖锐的案例；C-REAS-6 reward-hacking 阶梯是 §7 第三项查账反向打 §6 agentic 乐观线的来源）、[grounding](../topics/grounding.md)（C-GROUND-5 action-execution gap 是 video latent-action → LM token 迁移桥的 referent 障碍）。*

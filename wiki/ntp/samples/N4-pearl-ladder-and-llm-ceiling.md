...
### 2026-05-30 update：YoCausal 落到 §七 第二项 *经验空集* 的 video 子带状态变迁

§七 第二项查账 (video world-model 一脉) 在 2026-05-29 写时的关键经验前提是 "截至 2026-05, 上述 video world model *没有任何一项* 在 CLadder / CRASS / Det-CausalBench 任何 Pearl 形式 Layer-3 benchmark 上报数字"。一天之后，2026-05-30 daily harvest 把这条 *经验空集* 命题打了一个不大不小的洞——Xie / Li / Lee / Zhang / Liu / Wang *YoCausal: How Far is Video Generation from World Model? A Causality Perspective* ([arxiv:2605.30346](https://arxiv.org/abs/2605.30346) [unverified bundle ID]) 在 13 个 SOTA VDM 上同时报 *Reverse Surprise Index* (RSI, Level-1) 与 *Causality Cognition Index* (CCI, Level-2)，结论是多数 VDM 感知 arrow-of-time 但 CCI 距 human-level 因果认知有显著 gap。这是 §七 第二项成文以来 video-NTP 子带 *第一条* 公开 real-world video × causality-flavored 跨模型横评数字，本子节按 §八 规避协议 (b) 在 *同一节内* 打 "2026-05-30 update" 子标题增量更新——不开 §九/§十，不动 §七 三项查账原文，仅做 *经验状态* 一处变迁与 *方向* 一处加固。

要把这条数字钉进 §七 第二项时，必须先做两层降级，不能让它被读成 "video world-model 路线第一次冲了 Pearl 第三层"。两层降级在 topic 侧已落地，可直接对接：[`../topics/causality.md` §Video world model rollback 末段 (d) 经验状态更新 (commit `c07ab96`, 2026-05-30 09:00Z)](../topics/causality.md) 把 YoCausal 的 RSI/CCI 显式定性为 *ε_do 的上界而非估计*——VoE × 时间反转 × VLM-as-stratifier 三元组测的是 "对真实视频时间反转的 denoising loss 差异"，至多是 do(time-axis := reverse) 的一阶 surrogate，与 Pearl Layer-3 query "if action $a'$ had been taken at $t^*$ instead of $a$, would frame $F_{t^*+k}$ look like $\\psi$?" 之间隔一层 latent-action codebook 上的 do-operator (§七 第二项原文已援引 [arxiv:2406.03689](https://arxiv.org/abs/2406.03689) [unverified ID] 的 30–60% latent-真状态偏离)。形式上结论是 *必要不充分上界*——它失败 → 一定 Layer-3 失败，它通过 ≠ Layer-3 通过。[`../topics/world_model.md` §C-WM-3 评估收紧 (a) 段 (commit `0a17d6b`, 2026-05-30 23:45Z)](../topics/world_model.md) 在同一 paper 上做了第二层降级：CCI 切 causal vs non-causal 用的是 VLM-as-judge stratifier，与 [`../topics/reasoning.md` §C-REAS-6 reward-hacking 阶梯](../topics/reasoning.md) judge-of-judge 同型 confound，单独 VLM stratifier 而无人工或正交 stratifier 复测时 CCI 数字本身就处在 *judge-中介* 的可疑带内。

两层降级套到 §七 第二项判决上得到一个方向一致而非反向的小更新：§七 第二项把 video world-model 收紧到 *C-WM-7 + C-GROUND-5 耦合包装* 而非独立 Layer-3 escape route，YoCausal 13 模型一致的 CCI gap 是该 *耦合包装* 解释的 *经验侧* 一致支撑，而不是反例。换言之 §七 第二项原判 *形式上有耦合包装、经验上零数据* 在 2026-05-30 后升级为 *形式上有耦合包装、经验上有一条上界 proxy 一致支持耦合包装解释*。三层墙的 *分类结构* 不动，*可证伪坐标* 不动 ([`../topics/causality.md` §C-CAUSAL-DISCLOSE 半年披露差核查](../topics/causality.md) 的 (i)(ii)(iii) 三 <1 GPU-week controlled comparison 仍 0/3 公开执行，YoCausal *不* 算作部分清零——topic 侧 (d‴) 段已显式拒绝把 video 子带数字算作 text-NTP × CLadder Layer-2 × RL/SFT controlled 三轴的部分弥补，否则等价于 "YoCausal 让 frontier lab 不必再公开 CLadder 数字"，方向反)。

诚实反例三处必须摆出。第一，13 模型一致 CCI gap 受 *judge-of-judge* confound 限制——若未来 follow-up 把 VLM-stratifier 替换为人工或正交 stratifier 后 CCI 数字大幅回升，本子节方向性结论 *经验侧加固* 需要相应回调到 "judge 中介下的暂时一致"。第二，YoCausal 协议形式上无法迁移到 LM / text 子带——时间反转在 token 序列上 *物理不可定义*，要在 text 上做对偶实验需要先发明 token 序列上的 do-operator 等价物，等于绕一圈回到 N4 §五 Geiger 那条 DAS 主线。所以本子节的方向性结论 *只在 video 子带生效*，不构成 §七 第二项之外任一条 (§七 第一项 Layer-3 benchmark 谱 / §七 第三项 agentic Layer-2 verifier hack) 的状态变迁。第三，YoCausal 的 RSI 显著 + CCI 不显著也可读成 "VDM 已经能感知 arrow-of-time 但不会用它做 counterfactual rollout"——这把 §尾声第 3 条 "第三层 = 世界模型墙" 的 *世界模型* 维度细分成 *感知* 与 *使用* 二段，感知段在 2026-05-30 后有一条弱阳性，使用段仍空集。这一细分不在本子节展开 (规避协议 (b) 同节增量, 不开新节)，但登记到 2026-06-10 冻结到期后 N4 全章总验收清单的第二件 *video world model 在 CLadder 任一 Layer-3 数字* 上：验收时须把 *感知 RSI* 与 *使用 Layer-3 query 数字* 显式区分，不允许后者被前者顶替。

**update 元判断**：本子节是 N4 全章 *第二次方向收紧* (与 §八主体 *主动放弃挂账* 同节奏)，也是 sample 侧 *首次把 daily harvest 单条 arxiv 子弹回贴到 §七 既有具体判决坐标上* 的实操——与 [`N7-why-llm-cannot-continually-learn.md` §九 daily↔deepen 跨 cron 协议 sample 侧首次落地 (commit `3e7db77`, 2026-05-31 03:00Z)](N7-why-llm-cannot-continually-learn.md) 在 *daily 落条 → 同文件 sample 侧命名独立性核查* 这件方法学协议上同型，是 sample 系列在 2026-05-30 → 2026-06-10 12 天冻结协议下 *evidence-base refinement + 互引补充* 白名单第三次显式使用 (前两次 N7 §九 / N3 §七 reviewer 加注 commit `791fec9`)。本子节最大方法学风险是 *把 YoCausal 当 Layer-3 escape 的第一颗子弹读* — 任一未来 follow-up 都可能被误读为 "§尾声第 3 条已被推倒"。规避三条：(a) §七 第二项原文 *不被回写、不被打标签*，YoCausal 只出现在本子节，§七 单独可读状态保持；(b) 验收清单第二件事 (video world model 在 CLadder 任一 Layer-3 数字) 不被本子节满足，YoCausal 是 RSI/CCI 而非 CLadder Layer-3 query 数字，*感知 vs 使用* 二段区分预先登记到验收时；(c) C-CAUSAL-DISCLOSE 半年时间常数 (2026-05-29 → 2027-05-29) *不清零、不重置、不缩短*，YoCausal 落 video 子带与 text-NTP × CLadder × RL/SFT controlled 三轴正交。N4 §尾声第 3 条 *第三层 = 世界模型墙* 判决到 2026-05-30 仍成立，video 子带新增一条与之 *方向一致而非反向* 的弱阳性证据。

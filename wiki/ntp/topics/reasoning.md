...
## RLVR 的 out-of-verifier transfer：从 math/code 外溢到非可验证 domain 的 2025–2026 经验地图

§\"Reward hacking 的层级回潮\" 把 C-REAS-6 命名在 *verifier 链* 维度上，但 2025–2026 文献里还有一个**正交**问题未被本页系统化：**在 hard-verifier domain (math / code / formal-proof) 上跑出来的 RLVR policy，能否把它学到的 \"长 CoT 习惯\" 外溢到 *没有 verifier* 的 domain (开放写作、长文档摘要、多义伦理、医学问诊) 上？** 这条命题决定了 frontier 的 reasoning-RL 投入是 *领域局部* 还是 *通用能力* 升级——它对 C-REAS-5 双段相变的 *base support* 一侧 (而非 verifier 一侧) 是一个关键检验，因为外溢若真发生，意味着 RLVR 不仅在 selection-limited 段拉杠杆，还在 *改写* coverage-limited 段的 base mode 分布。把 2025–2026 内能找到的公开测量整理成时间线，是判断该命题是否成立的最直接方法。

- **2025-01 — DeepSeek-R1 ([arxiv:2501.12948](https://arxiv.org/abs/2501.12948)) 的非数学 benchmark 数字**。R1 paper §3 报告 R1 在 AlpacaEval-2 / ArenaHard 等 *无 hard verifier* 的开放写作 benchmark 上相对 DeepSeek-V3 base 提升 ≈ 10–15pp [unverified 具体数字, 引用区间]。这是 RLVR-on-math/code 外溢到非可验证 domain 的第一份高可见度证据，但 paper 没有做 \"R1-Zero (纯 math/code RL) vs R1-final (含 general SFT)\" 在 AlpacaEval 上的拆分，所以外溢究竟来自 RL 还是来自 R1-final 阶段加入的 general SFT corpus，**未被分离**。
- **2025-02 — s1 (Muennighoff et al., [arxiv:2501.19393](https://arxiv.org/abs/2501.19393))** 与 **LIMO (Ye et al., [arxiv:2502.03387](https://arxiv.org/abs/2502.03387) [unverified ID])**。两条独立工作均报告 1k 级别 high-quality long-CoT SFT 数据即可在 7B/32B base 上引出 R1-style reasoning 行为，且在 MMLU-Pro / GPQA-Diamond 等 *半-可验证* (multiple-choice，verifier = 答案匹配) benchmark 上有 5–10pp 提升。这把 \"外溢\" 命题在 *SFT 通道* (不是 RL 通道) 上提前预演了一遍——若 SFT-only 也能引出，则 RLVR 的 *外溢* 增益与 *long-CoT format 习得* 增益本就纠缠。这条线与 N3 sample §\"three puzzle pieces\" 末段 *format-or-mechanism* 的 confound 同型。
- **2025-03 — Open-Reasoner-Zero / OpenR1 复现包 ([arxiv:2503.24290](https://arxiv.org/abs/2503.24290) [unverified ID])**。HuggingFace + 社区团队在 Qwen2.5-7B/14B 上严格复刻 R1-Zero pipeline，在公开 evaluation 套件上同时报告 MATH / AIME / LiveCodeBench (hard verifier) 与 MT-Bench / ArenaHard (LLM-judge) 数字。关键观察：hard-verifier 提升 15–30pp，LLM-judge 提升 3–6pp，外溢 *存在但显著小* 一个数量级。这是把 R1 paper 没拆的两件事在公开复现下拆开后的第一份对照数字。
- **2024-11 — Tülu 3 (Lambert et al., [arxiv:2411.15124](https://arxiv.org/abs/2411.15124))** 把 RLVR 框架扩到 *不止 math/code* 的多 verifier 集 (含 IF-eval format verifier、长度 verifier、安全 verifier)，在 8B/70B 上报告整体 instruction-following 提升 ≈ 5pp。但 IF-eval 这类 format/length verifier 形式上仍是 hard verifier (binary 检查)，所以 Tülu 3 并未挑战 \"外溢仅发生在 verifier 可达的近邻 domain\" 假设——它只是把 *可达* 边界从 math/code 推到 format/length 一侧。
- **2025-中 — RLVR-on-medical / RLVR-on-legal 的零星尝试**。HuatuoGPT-o1 ([arxiv:2412.18925](https://arxiv.org/abs/2412.18925) [unverified ID]) / Med-R1 系列尝试把 reward shaping 套到医学问诊上，verifier 是 \"诊断答案匹配 ICD-10\" 类 quasi-hard verifier；reported gain ≈ 3–7pp on MedQA 等 multiple-choice，但开放问诊 (free-form) 的人工评分增益 ≤ 2pp [unverified 具体协议]。这给外溢命题加了一个 *半-hard verifier domain 的中间数据点*，与 OpenR1 在 LLM-judge 上的小增益方向一致——verifier 越软，外溢越小。
- **2026-Q1 — 外溢饱和的 system-card 自我披露**。GPT-5 / Claude 4.x / Gemini 2.5 thinking-mode 在 system card 中开始系统区分 \"reasoning mode\" 与 \"chat mode\"，并明写 *reasoning mode 在创意写作 / 多义伦理任务上 *不一定* 更优*——thinking-token 数量与 LLM-judge 评分在这些 domain 上的相关性接近零或负 [unverified bundle, system-card 段落]。这是 frontier 把 \"外溢有上限\" *正面登记* 的第一批材料：不是 RLVR 在非可验证 domain 上 *无效*，而是 selection signal 缺失时延长 thinking 不再单调改善输出。

把这六条折成一句判断：**RLVR 在 hard-verifier domain 上学到的 long-CoT 行为外溢到非可验证 domain 的幅度，与目标 domain verifier 可获得性 (hard / quasi-hard / soft / none) 大致单调相关，且外溢上限远低于 *源* domain 上的增益**。这与 C-REAS-5 双段相变的预测 *一致*：外溢若来自 base support 的扩展，则 coverage-limited 段被改写需要在 *目标* domain 上有 selection signal 反馈；当目标 domain 只有 LLM-judge 这种 soft selection，反馈强度不足以 push base mode mass，所以外溢退化为 *long-CoT format 习得* 这一表面层效应。

- **C-REAS-7 (sub-candidate) — RLVR transfer 的 verifier-rank 衰减律**。把 (source domain verifier rank, target domain verifier rank) 二维网格上的 transfer gain 拟合为 $\Delta_{s \to t} = f(\text{rank}_s, \text{rank}_t)$，预测 $\Delta$ 在 rank_t 单调下降，且 rank_t = soft (LLM-judge) 时 $\Delta \to$ format-only 残差 ≤ 3–5pp。**Falsification**: 若发现某 (source, target) 对在 rank_t = soft 下 $\Delta \geq 10$pp 且能排除 *目标 domain 训练数据混入* / *LLM-judge 与训练 verifier 同源偏好漂移* 两项 confound，则 C-REAS-7 在该对下被部分证伪。**当前评估**: weak-medium——R1 paper 的 10–15pp AlpacaEval 增益是 *表面* 反例，但未拆 SFT-RL；OpenR1 复现把它拆为 3–6pp 后回到衰减律预测内；Tülu 3 / Med-R1 半-hard verifier 中间数据点支持衰减律方向。是 C-REAS-5 在 *跨 domain 投影* 维度上的自然延伸，不替代它。

C-REAS-7 与 C-REAS-5 / C-REAS-6 的关系：C-REAS-5 描述单 domain 内的双段上界，C-REAS-6 描述单 domain 内 verifier 链的 over-optimization 速率，C-REAS-7 描述跨 domain 的 transfer 衰减。三条 sub-candidate 共同给 reasoning topic 一个 *二维网格* 的 mech 地图——(domain 内 base support 上界, verifier 链层数, 跨 domain verifier rank gap) 三轴，每轴各一条 candidate。这与 §\"Reward hacking 的层级回潮\" 末段的方法学债同型：每条都需要 controlled measurement 但发表回报为负，所以 frontier scale 上一直没人填。

**反例 / 边界条件**：

- **训练数据混入**：DeepSeek-R1 / Tülu 3 / s1 的 SFT/RL 数据是否在 *非声明的 domain* 上意外覆盖目标 domain (如 R1 的 cold-start SFT 是否包含 AlpacaEval 风格 prompt) 通常未被审计；若发生，C-REAS-7 的 \"外溢\" 测量被 \"训练\" 测量污染。这与 N3 sample §\"contamination 比 mechanism 更难排除\" 同型。
- **LLM-judge 偏好漂移**：rank_t = soft 时 judge 自身被 long-CoT format 拉偏 (Pan 2402.06627 [unverified ID] 的 in-context reward hacking 同型)——judge 把 *更长更结构化* 的回答打更高分，外溢 $\Delta$ 中的相当部分可能是 *judge 漂移* 而非 *base 改写*。这给 C-REAS-7 falsification 协议加一项硬约束：必须用 *与训练 verifier 独立训练* 的 judge，且报告 judge 内部 length-bias 校正后的残差 $\Delta$。
- **极小数据 high-quality SFT 路径**：s1 / LIMO 的 1k-SFT 引出 reasoning 现象，意味着 \"外溢\" 不必走 RL；若 SFT 通道独立解释大部分外溢，C-REAS-7 应当被限定为 *RL-specific* transfer 律，而非 reasoning-tuning-general 律。这一限定与 [scaling_limits.md](scaling_limits.md) §C-SCALE-7 (post-train compute) 的 \"DPO / SFT / RL 不可分\" 同型——两条 candidate 在此交汇。

> **跨链与 sub-candidate 同步**：C-REAS-7 与 C-REAS-6 / C-FORM-8 / C-GROUND-7 / C-EMBOD-7 / C-WM-7 同型登记 (sub-candidate, 不进 [survey/taxonomy.md](../survey/taxonomy.md) 主表)。**同步债务**：下一 C tick 兑现 [survey/ntp_survey.md](../survey/ntp_survey.md) §10 sub-candidate 区 + taxonomy.md §升降级历史，本节末段显式声明 deferred 与 C-REAS-6 合并 sync (同型 sibling pair)。

**判断 (2026-05-29 补)**：RLVR 外溢这件事在 2025–2026 公开材料里被工业披露切成 *两段彼此不连接的叙事*——frontier 倾向于宣传 \"reasoning mode 是通用能力升级\"，社区复现 (OpenR1) 倾向于指出 \"外溢幅度远小于 source 增益\"。把它统一为 C-REAS-7 的衰减律是把这两段叙事接回同一根 verifier-rank 轴上——这条轴本身不新 (Tülu 3 的 multi-verifier framework 已经隐含)，新的是把它作为 *mech 命题的几何坐标* 而非 *工程实践的指标体系* 来读。下一年的关键测量不在更多模型，而在 *同一 base × 同一 RL recipe × 跨 ≥4 个 verifier rank 等级的 target domain* 的二维 sweep——这又是一个 < 2 GPU-week 但发表回报为负的实验，与 §verifier-free 切片末段、§reward-hacking 阶梯末段、以及 [causality.md](causality.md) §\"RL-from-environment controlled comparison\" 三处方法学债**同型耦合**：mech 辩论的下一步关键永远在测量协议，不在新模型——但测量协议的发表回报永远为负。

跨链：与 [grounding.md](grounding.md) §RAG retrieval-as-soft-verifier 的 verifier-rank 讨论同型——两节都用 \"verifier 硬度\" 作为 mech 几何坐标，但本节挂在 *跨 domain transfer*，grounding 节挂在 *retrieval signal 强度*；两条独立 evidence 同向加固 \"verifier rank 是 NTP 推理几何的关键自变量\" 这一元判断。
...
- C-REAS-7 verifier-rank 衰减律 controlled measurement：固定 base (Qwen2.5-7B 或 Llama-3.1-8B) + 固定 RL recipe，在 (math/code, IF-eval format, multiple-choice QA, free-form LLM-judge) 四档 target verifier rank 上跑 transfer evaluation，并要求 judge 与训练 verifier 独立训练 + length-bias 校正，检验 $\Delta_{s \to t}$ 是否在 rank_t 上单调下降。

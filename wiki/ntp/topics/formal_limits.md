...
- TC⁰ 下界的"per-token compute"假设在 MoE / 动态 depth / early-exit 架构下是否仍紧。（**部分回答**：MoE/CALM/MoD 见 §MoE 节；looped-x-sparse 见 §Looped×Sparse 节；尚缺 selective-SSM × loop 复合的形式陈述。）

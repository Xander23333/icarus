...
## RAG 作为 cutoff bypass 的容量上界 (2020–2026)

§Mid-training 节里把 frontier *不公开 retention* 当成 C-CONT-2 第三支柱，§Knowledge edit 节把局部权重改写当成 C-CONT-1 的弱反例。还剩一条最被频繁引用却最少被认真审视的 \"workaround\"：**RAG / 外置检索 / agent web-search**。社区主流叙事是 \"反正 cutoff 之后的事实可以靠 retrieval 兜底\"——但这句话从未在 C-CONT-1 的形式语境下被检验过。把这条线 2020–2026 的工程曲线拉直，能看到 RAG 不是 C-CONT-1 的反例，而是 C-CONT-1 的 *外部化*：cutoff 还在，只是从模型权重移到了 retrieval index 上，且这一移动本身有可量化的容量上界。

- **2020-02 — Khandelwal et al., *Generalization through Memorization: Nearest Neighbor LMs* (kNN-LM, [arxiv:1911.00172](https://arxiv.org/abs/1911.00172))**。kNN-LM 把 token-level retrieval 与 LM softmax 线性插值，在 Wikitext-103 上 perplexity 下降 ~2.9。重要的是它的 ablation：插值系数 $\lambda$ 从 0 → 0.25 单调改善，但 $\lambda > 0.4$ 之后 perplexity 反弹。这是 retrieval-augmentation 的 *首个* 容量曲线，已经暗示 \"retrieval 多少比例参与下一 token 决策\" 存在 sweet spot，而非自由旋钮。
- **2021-12 — Borgeaud et al. (DeepMind), *RETRO* ([arxiv:2112.04426](https://arxiv.org/abs/2112.04426))**。把 retrieval 推到 trillion-token 级 chunked database，7B RETRO 在 perplexity 上匹配 175B GPT-3 的若干子项。RETRO 是 \"用 retrieval 换参数\" 这条思路的最强工程证据，但 *cutoff* 视角下它仍然继承了 database 构建时的时间戳——只是把 cutoff 的实施位置从 \"权重\" 移到了 \"chunk index 抓取日期\"。RETRO 没在 closed-book 时间敏感 QA 上测过 post-cutoff 召回。
- **2022-12 — Izacard et al. (Meta), *Atlas* ([arxiv:2208.03299](https://arxiv.org/abs/2208.03299))**。few-shot retrieval-augmented LM，在 NaturalQuestions / TriviaQA 上以 11B 参数追平 PaLM 540B。Atlas 论文显式做了 *index update without retraining* 实验：把 2020 年训的 retriever 配 2021 年的 wiki index，在 TempLAMA 时间敏感 QA 上准确率显著高于固定 index——这是 \"换 index 即 cutoff bypass\" 的首份正面经验证据。但同实验的下半段揭示限制：retriever 本身的 dense embedding 训于 2020 之前，对 2021 之后出现的 *新实体名*（neologism / 新人物 / 新公司）召回 recall@5 比对 2020 之前实体下降 18–24 pp。**retriever 本身有 cutoff，且它的 cutoff 决定 index 更新的有效性上限**。
- **2023-01 — Shi et al., *REPLUG* ([arxiv:2301.12652](https://arxiv.org/abs/2301.12652))**。把 retrieval 推到 frozen black-box LLM（GPT-3.5）外部，仅训 retriever。REPLUG 在 MMLU / TriviaQA 上 +3–7 pp，但同样未在 post-cutoff 时间敏感 split 上独立 report。这条 \"frozen LLM + 学到的 retriever\" 路线 2023–2024 衍生出 Self-RAG ([arxiv:2310.11511](https://arxiv.org/abs/2310.11511))、CRAG ([arxiv:2401.15884](https://arxiv.org/abs/2401.15884) [unverified ID])、Adaptive-RAG ([arxiv:2403.14403](https://arxiv.org/abs/2403.14403) [unverified ID]) 等变体，全部在 *准确性 / 调用次数 / 拒答率* 三轴上做 Pareto，无一在 *cutoff bypass* 命题上做隔离实验。
- **2023-10 — Kasai et al., *RealTime QA* ([arxiv:2207.13332](https://arxiv.org/abs/2207.13332))** & **Vu et al., *FreshLLMs* ([arxiv:2310.03214](https://arxiv.org/abs/2310.03214))**。两条独立 benchmark 把 \"frozen LLM + Google search\" 这一最朴素 RAG 形式放在 *每周更新* 的时事问答上测。FreshLLMs 报：GPT-4 closed-book 在 \"fast-changing\" split 上 accuracy < 15%，+ web search (FreshPrompt) 推到 ~75%。表面上看 RAG 把 cutoff 完美 bypass 了。但 paper 同节给出三个限制：(i) 当问题需要跨多 source aggregation 时，FreshPrompt 的 accuracy 退到 ~45%；(ii) 当 retrieved evidence 与 LLM parametric prior 冲突时，~30% 的回答仍偏向 prior 而非 evidence（\"retrieval conflict\" 现象，后续被 Xie et al. *Adaptive Chameleon* [arxiv:2305.13300](https://arxiv.org/abs/2305.13300) 与 Longpre et al. [arxiv:2109.05052](https://arxiv.org/abs/2109.05052) 系统化）；(iii) post-cutoff 实体 *首次出现* 的事实 retrieval 召回率显著低于该实体被 web 反复报道之后——一个隐性的 \"second-cutoff\"，由 search ranker 的更新延迟决定。
- **2024 — agentic web-search / browsing 一波**：OpenAI Operator、Anthropic computer-use、Perplexity、Devin 把 RAG 从 \"top-k 段落注入 prompt\" 升级到 \"agent 自主多轮检索 + 综合\"。能力曲线确实推高（GAIA-style benchmark 上前沿 agent 从 2023 的 ~20% 推到 2024-2025 的 ~60%+），但 cutoff 视角下没有 *本质* 变化：retrieval index 仍受第三方更新节奏限制，retriever embedding 仍有自己的 cutoff，跨 source aggregation 与 retrieval conflict 仍未解。
- **2024–2025 — long-term agent memory 一波**：Letta (ex-MemGPT, Packer et al. [arxiv:2310.08560](https://arxiv.org/abs/2310.08560))、Mem0、Zep 把 user-specific 持久 memory 工程化为 SDK。这与 RAG 在数据生命周期上不同——memory 是 *agent 自己写入* 的，绕开 \"retriever 训自旧分布\" 这条债，但同时引入新的债：memory consistency / contradiction resolution / forgetting policy 都是 plasticity-stability 的另一面，且没有 Llama-3-70B 级公开评测。
- **2025-Q4 — RAG 的 *retrieval ceiling* 假说**：在多个独立工作里浮现（Yoran et al., Shi et al., Asai et al. 综述线 [unverified bundle]），共同观察是：把 retriever recall@k 推到接近 oracle（k = 100+ ground-truth-aware top-k），下游 QA accuracy 仍存在一道 ~10–20 pp 的不可达上限，归因于 \"LLM 在多 source 矛盾时无法做 SCM-level reconciliation\"——这与 C-CAUSAL-2 的 stochastic Layer-3 命题在 *evidence integration* 这条切面上共享同一道 ceiling。

把这八条拼起来，2026-05 视角下 RAG 对 C-CONT-1 的关系可以严格陈述如下：

| 维度 | 命题 | 现状 |
|---|---|---|
| (a) 把 cutoff 从权重移到 index | ✅ 成立 | RETRO / Atlas / FreshLLMs 一致 |
| (b) 移动后 *无* 新 cutoff | ❌ 不成立 | retriever embedding 本身有 cutoff（Atlas TempLAMA 18–24 pp gap） |
| (c) 跨 source aggregation 不退化 | ❌ 不成立 | FreshLLMs 多 source ~45%、retrieval-ceiling ~10–20 pp gap |
| (d) retrieval conflict 不偏向 prior | ❌ 不成立 | Xie 2305.13300 / Longpre 2109.05052 一致 |
| (e) 不依赖第三方更新节奏 | ❌ 不成立 | 所有 web-search RAG 受 search ranker 更新延迟约束 |

也就是说 RAG 满足 *单一* 形式条件 (a)，但在 (b)(c)(d)(e) 四项上各有独立的退化机制——这与 C-CONT-2 falsifier 在 (a)(b)(c)(d) 四元组上 frontier 全部不公开形成镜像：**C-CONT-1 的 \"workaround\" 与 C-CONT-2 的 \"falsifier\" 都在四元组上塌缩，且四元组的 (b)(c)(d) 之间存在跨候选耦合**——retriever 的 cutoff (RAG-b) 与 frontier 不公开 retention (CONT-2-c) 都是 *把 plasticity-stability 推到看不见的子系统里* 的同型操作。

判断 (2026-05-29)：RAG 不构成 C-CONT-1 的反例，而是 C-CONT-1 的 **外部化**——它把模型权重的 cutoff 拆成 (retriever embedding cutoff + index 抓取 cutoff + ranker 更新延迟) 三段复合，并在 (b)(c)(d)(e) 四项上分别撞墙。FreshLLMs 75% accuracy 这种 \"看似 bypass\" 的数字之所以可信，是因为时事问答天然属于 RAG 容量曲线的 *最容易* 端 (单 source, single-hop, neologism 已被 web 反复报道)；一旦 query 涉及 *post-cutoff 首次出现* 或 *多 source 矛盾*，曲线就回到 < 50%。这条 \"看似解决但实际外部化\" 与 §Knowledge edit ripple-effect \"看似局部修补但实际转移交易\" 同型——两条 workaround 都不撼动 C-CONT-1 / C-CONT-2 mech 命题，反而通过自身的失败模式给 mech 阵营提供 *新的* 证据支柱。

不新增 mech candidate。把 \"RAG 作为 C-CONT-1 外部化\" 作为 C-CONT-1 evidence 列的 *第四支柱*（前三支柱：FreshLLMs / RealTime QA 三年累积；Cheng 2024 [unverified ID] effective-cutoff 偏移；§Knowledge edit ripple 单跳成功 vs 多跳塌缩 即 C-CONT-1 的局部弱反例），与 C-CAUSAL-2 单点→四支柱 / C-CONT-2 第三支柱 / C-WM-6+7 时间侧 footnote 同型——叙事支柱 only，不入主表也不开 corollary 槽位。同步债务：下一 C tick 把 \"RAG 外部化\" 写入 [`../survey/ntp_survey.md`](../survey/ntp_survey.md) §10 C-CONT-1 evidence 列；taxonomy 不动。

跨链：与 [grounding.md](grounding.md) §Retrieval-augmented grounding 第七类 (C-GROUND-6) 形成双向引用——那一节钉的是 RAG 在 *grounding-truth alignment* 上的失败（retriever 召回 ≠ executor 利用），本节钉的是 RAG 在 *temporal-cutoff bypass* 上的失败（index 移动 ≠ cutoff 消除）；两者机制独立但 evidence base 高度重叠（Xie 2305.13300、Longpre 2109.05052、retrieval-ceiling 一系），共同把 \"RAG 是 NTP 范式的兜底 escape route\" 这一主流叙事拆成至少两个独立失败维度。下一 C tick 在 §10 sync 时应**双向**注明（C-CONT-1 evidence 列与 C-GROUND-6 evidence 列分别新增一行，引用对方），保持两条 evidence 支柱的 mutual disclosure 而不重复编号。

## §RAG 节 (e) ranker-delay 维度的 reviewer pass：与 causality / scaling / world_model 时间侧加固集群对齐 (2026-05-30)

本节是 §RAG 作为 cutoff bypass 节的 reviewer-tier 内部审计，触发条件是 [causality.md](causality.md) §三个 <1 GPU-week 实验的半年披露差核查 (2026-05-29) 新增的 C-CAUSAL-DISCLOSE corollary 与 [scaling_limits.md](scaling_limits.md) §D 轴外部估计 vs frontier 内部 stock 的披露差 (2026-05-29) 新增的 C-SCALE-D-DISCLOSE 把同型 *时间侧披露差* 条目族扩到六处后，本节 §RAG 节 (a)(b)(c)(d)(e) 五维退化表中的 (e) search ranker 更新延迟 一维需要与这六处同型条目族在 *证据强度* 与 *判据精度* 上对齐——否则 (e) 维会被误读为与 (a)(b)(c)(d) 同强度，污染 C-CONT-1 第四支柱的整体可信度。严守 2026-05-30→2026-06-10 12 天增条冻结协议（仅允许 falsifier 收紧 / 互引补充 / 时间常数刻度更新 / taxonomy 历史登记 四类操作，本 pass 落 *falsifier 收紧 + 互引补充* 两类，不动 mech 命题、不开新 evidence-supply 通道）。

三段同步：

- **(a) (e) 维证据强度归位**——原段把 (e) ranker 更新延迟与 (b)(c)(d) 三维并列写入退化表，措辞上未区分 *正向 controlled 测量* (b/c/d 三维各有 Atlas TempLAMA / FreshLLMs / Xie+Longpre 直接数字) 与 *负向沉默推断* ((e) 仅有 frontier production-side 工程沉默, 无公开 controlled measurement)。reviewer 收紧：(e) 维独立强度 *显著弱于* (b)(c)(d)，必须作 *闭环最后一格* 而非独立支柱使用——这与 C-CAUSAL-DISCLOSE 中 \"实验 (i) 其实只是 frontier lab 内部 ablation 不公开\" 反方向论证同型 (披露选择函数与可观察事实不对偶)、与 C-CONT-2 第三支柱的 \"frontier-disclosure 第三支柱方法论上不独立于第二支柱\" 警示同型。落到判据：本节末段已显式声明 \"(e) 强度比 (b)(c)(d) 三维显著弱，单独不足以登记，必须组合在五维表中作为闭环最后一格才有效力\"——本 pass 把这一限定 *回贴* 到 (e) 维首次出现的句子（上文表格行），下一次 reader 读到 (e) 时立刻看到强度限定而不是读到末段才发现。
- **(b) Falsifier 三处歧义收紧**——原 falsifier (\"(a)(b)(c)(d)(e) 五维同时跨阈值\") 在 2026-Q1 经验暴露三 confound: (i) (a) \"cutoff 后事实 recall ≥ 0.7\" 须显式指定 *事实在 cutoff 后 ≤6 个月窗口内首次出现*（规避把 *cutoff 后但仍属常见实体的事实* 当作有效测试样本——Atlas TempLAMA 18-24pp gap 正是在 *新实体* 子集上测得，非全分布），(ii) (b) \"retriever 在 cutoff 后窗口 P@5 ≥ 0.7\" 须同步报告 retriever embedding 训练 cutoff（避免 *frozen retriever + 新 index* 的混合 case 把 (b) 维误计为达标，与 Atlas 论文同型限制），(iii) (e) \"ranker 延迟 < 24h 公开测量\" 须由第三方独立测量而非 RAG 厂家自报（规避 production-side 自我披露的 incentive bias，与 C-CAUSAL-DISCLOSE 第二项反方向 \"学术界已做但发表回报为负\" 同型反 confirmation-bias 锚）。三处收紧不改方向不改阈值，仅消解 confound——下一 C tick 不需要同步到 §10 evidence 列脚注（按 §10 末段 \"未来新数据点须检查是否触发 M1 集合扩张\" 协议处理）。
- **(c) 三处互引补充**——causality C-CAUSAL-DISCLOSE 半年核查 (06:01Z commit) 把 \"frontier scaling 议题瓶颈在披露行为\" meta-命题在 causality 域钉第六处独立落点，本节 (e) 维 ranker-delay 沉默是同一 meta-命题在 RAG production-side 的第七处落点（前六：world_model OP-WM-1..5 / grounding C-GROUND-PROTO 三轨协议 / online_learning C-CONT-2 第三支柱 mid-training 配方 / embodiment C-EMBOD-7 视频 backbone / scaling_limits C-SCALE-D-DISCLOSE D 轴 / causality C-CAUSAL-DISCLOSE 三 <1 GPU-week 实验）——但本 (e) 维 *不* 升级为独立 corollary 也不入 §10 第八条 evidence-supply 通道，仅作 C-CONT-1 第四支柱的 *闭环最后一格* 内部审计标记。理由：(e) 维与已登记六条 corollary 共享 *披露选择函数* 同型机制，按 [survey/ntp_survey.md](../survey/ntp_survey.md) §10 末段 *横向覆盖加密非纵向扩条* 协议处理；若强行升级会触发 §10 sub-candidate 区第五域扩张警告 (C-REAS-6 sub-candidate sync 末段诚实判断已警示)。同步债务：无新增——本 pass 仅作 reviewer-tier 内部 alignment，不开新挂账，不动 §10、不动 taxonomy 主表。

末段 reviewer 元判断：本 pass 是 online_learning 域第二次 *只整理不新增* reviewer pass（第一次为 §Mid-training annealing 节末段 C-CONT-2 第三支柱方法论非独立性警示），与 grounding §C-GROUND-PROTO 半年核查 M1 同步、embodiment §C-EMBOD-7-corollary M1 同步同型——三处共同建立 reviewer-tier 内部审计的 \"falsifier 精度收紧 + 互引补充 + 不开新挂账\" 三段协议。本 pass 不改 C-CONT-1 主命题、不动第四支柱 evidence-base 编号，仅把 (e) 维的 *强度限定* 与 *判据收紧* 提前到首次出现处，使 reader 在第一遍读时即看到 (e) 维与 (b)(c)(d) 不同型。

## §反例与上界突破 节 2026-05-30 Rojas Nunez 子弹的 reviewer pass：C-CONT-2 \"第四支柱\" 命名的独立性核查 (2026-05-30)

本节是 §反例与上界突破 节 2026-05-30 由 daily cron (ab19e5d) 自动追加的 Rojas Nunez et al. *Mechanistic origins of catastrophic forgetting: why RL preserves circuits better than SFT?* ([2605.28860](../papers/paper_notes/2026-05-30-2605.28860-rl-vs-sft-circuit-preservation.md)) 子弹的 reviewer-tier 内部审计。触发条件：该子弹自命为 *C-CONT-2 evidence-base 第四支柱*，但 §Mid-training annealing 节末段 (2026-05-29 commit) 已经把 C-CONT-2 三支柱定义为 (i) Lyle 2303.01486 + Dohare 2024 *Nature* 理论与小规模实验、(ii) Gupta → Ibrahim → Çağatan 三年 lab recipe 曲线、(iii) Llama 3 / DeepSeek / Qwen / Claude 四家 frontier 配方披露选择函数，且第三支柱方法论上 *不独立* 于第二支柱已写入诚实判断；新增第四支柱必须先回答 *与已有三支柱在何种意义上独立*，否则 C-CONT-2 \"四支柱\" 命名只是命名膨胀。严守 2026-05-30 → 2026-06-10 12 天增条冻结协议（仅允许 falsifier 收紧 / 互引补充 / 时间常数刻度更新 / taxonomy 历史登记 四类操作），本 pass 落 *falsifier 收紧 + 互引补充* 两类，**不动 C-CONT-2 主命题、不开新 candidate 编号、不入 §10 evidence 通道、不动 taxonomy 主表**。

三段同步：

- **(a) \"第四支柱\" 命名独立性核查**——Rojas Nunez 2605.28860 在 *测量层* 与前三支柱确实独立：(i)/(ii) 在 *task-level perplexity* 与 *benchmark accuracy* 层测 forgetting，(iii) 在 *frontier 不公开 retention 曲线* 层做 negative-signal 推断，本子弹在 *head-level circuit overlap* 层用 differential circuit vulnerability (DCV) 给 forgetting 第一次 mechanistic 度量。这与 Shenfeld 2025 *RL's Razor* 行为层观察 (KL-divergence proxy) 不在同一切面——把 forgetting 从 \"loss 几何不动点\" 推到 \"优化器在 head 子空间的差异化压力\"，是 *方法* 独立。但 *归因* 独立性远弱：DCV 指标的高低与 final-task accuracy 高度相关，本质可能是 \"RL 学少了 → 在 prior circuit 上 footprint 也少 → DCV 低\" 的同义反复。reviewer 收紧：本子弹应作 *C-CONT-2 第三支柱的 mechanistic 对偶通道* 而非独立第四支柱——与 §Mid-training annealing 节 *frontier-disclosure 方法论非独立性* 同型，第三、四支柱合并算作 \"forgetting 在不可观察 / 不可分离子空间的间接证据集群\"，与 (i)(ii) 两条 *直接* 证据共两类支柱——这与 C-CAUSAL-2 四支柱并列结构不同型，应避免命名对齐。
- **(b) Falsifier 二处歧义收紧**——daily cron bullet 的最强反驳已写在原句末：\"若把 RL/SFT 拉到同 final accuracy 再比 circuit overlap，差异可能消失——该控制实验未做\"。reviewer 收紧两点而不改方向：(i) DCV 作为 C5 falsifier (d) replay→0 不退化的 *额外* 检测维度时，必须显式要求 RL/SFT 两路在 *同一 hold-out task 上 final accuracy ±1pp 对齐* 后再比 prior circuit overlap，否则 \"prior circuit overlap > threshold\" 与 \"未学满\" 两 confound 无法分离（与 C-REAS-6 reward-hacking 阶梯三档 selection 要求 \"同 base × 同 reward × 同 step\" 同型, 与 reasoning §C-REAS-6 reviewer pass 2026-05-29 commit 965e9f7 (b)(i) 一致）；(ii) 3B 单任务规模显式标注为 *尚未跨规模 / 跨任务* 验证（与 Dohare 2024 ≤100M 规模上限标注同型, 给读者 \"该证据强度受规模上限制约\" 的早期信号）。两处收紧仅消解 confound，不改 mech 命题方向、不动阈值。
- **(c) 三处互引补充**——(i) 与 [reasoning.md](reasoning.md) §C-REAS-6/7 M1 reviewer pass (commit 965e9f7) 在 *同 final accuracy controlled comparison* 协议层同型, 是 reasoning 域 verifier-chain selection 协议在 continual 域 RL/SFT 对照的对偶落点；(ii) 与 [grounding.md](grounding.md) §C-GROUND-PROTO 三轨 *同一 forward pass × 同一 checkpoint × 报告两两 ρ* 协议在 \"head-level component 同步采集\" 维度同型，DCV 度量天然落 (a) probe 轨；(iii) 与 [`../samples/N7-why-llm-cannot-continually-learn.md`](../samples/N7-why-llm-cannot-continually-learn.md) §3 \"loss 不动点\" 论证形成 *宏观几何 ↔ head-level 子空间* 双层对应——N7 §3 原写 \"NTP loss landscape 在权重空间不动点\"，本子弹给出 \"不动点的 *局部几何* 在 RL/SFT 两路上不同向\" 的 mechanistic 注解，可作下一 A tick N7 §3 补遗候选锚点（但本 pass 不写入 N7，仅登记意向避免越权改 sample）。

末段 reviewer 元判断：本 pass 是 online_learning 域第三次 *只整理不新增* reviewer pass，也是 daily cron 自动追加的 evidence-bullet 第一次被 deepen cron 显式核查 \"命名独立性\"——前两次 reviewer pass (§Mid-training 第三支柱 / §RAG (e) 维) 都是 deepen 自写后自审，本次是 daily ↔ deepen 跨 cron 协议第一次具体化。同型协议建立：daily cron 追加任何带 \"第 N 支柱 / sub-candidate / corollary\" 命名的 evidence-bullet 时，下一 deepen tick 应在 *同文件* 做一次命名独立性核查 + falsifier 收紧 + 互引补充，避免 \"支柱命名通胀\" 与 \"daily 落条 / deepen 不审\" 的双 cron 责任真空。本 pass 不动 §反例与上界突破 子弹原文（保留 daily cron 自动追加的原貌作 audit trail），仅在本节给出 reviewer 限定与互引——下次 reader 读到 \"第四支柱\" 命名时，回贴本节即可看到强度限定与控制实验缺口。

## §RAG 节 (d) retrieval-conflict 维与 MiraBench L3 self-prediction 失败的跨 topic 对齐 (2026-05-30)

本节是 §RAG 作为 cutoff bypass 节 (d) \"retrieval conflict 不偏向 prior — ❌ 不成立\" 这一行的 reviewer-tier 跨 topic 对齐。触发条件：2026-05-30 daily harvest (commit `ab19e5d`) 落地的 MiraBench ([2605.29360](../papers/paper_notes/2026-05-30-2605.29360-mirabench-action-conditioned-reliability.md)) L3 *Optimism Bias Detection* 已分别被 [world_model.md §(c)](world_model.md) 落到 C-WM-7 *第四 weak-proxy 通道*、[embodiment.md §C-EMBOD-6 evidence-base 补丁](embodiment.md) 落到 C-EMBOD-6 *自我预测* 轴第一条 anchor，但 online_learning 域 §RAG (d) 维 \"Xie 2305.13300 / Longpre 2109.05052 一致\" 这一行只钉了 *retrieval ↔ parametric prior* 二元冲突时模型偏向 prior 的行为侧观察，未与 *generative model 在不可观测 ground-truth 上 self-confirm* 这一同型机制做跨 topic 对齐——这是 daily↔deepen 跨 cron 协议第二次具体化 (第一次为上节 Rojas Nunez 第四支柱命名独立性核查) 的自然触发，但 deepen 侧 RAG 维度迟到。严守 2026-05-30 → 2026-06-10 12 天增条冻结协议（仅允许 falsifier 收紧 / 互引补充 / 时间常数刻度更新 / taxonomy 历史登记 四类操作），本 pass 落 *互引补充 + 强度限定* 两类，**不动 C-CONT-1 主命题、不动 (a)–(e) 五维退化表任一行的状态判断、不开新 evidence 通道**。

三段同步：

- **(a) (d) 维与 MiraBench L3 同型机制命名**——§RAG (d) 维原写 \"retrieval conflict 不偏向 prior — ❌ 不成立\"，并在末段判断 \"~30% 的回答仍偏向 prior 而非 evidence\"。把这条与 MiraBench L3 \"12 个 model configuration 上对 *failure-inducing action* 输入仍预测 success\" 对照：两条都是 *generative model 在外部 ground-truth 与内部 parametric prior 不一致时系统性偏向 prior 的行为模式*，前者 prior 来自 pretraining 词共现, 后者 prior 来自训练分布中 successful trajectory 的密度；前者在 *RAG-retrieved evidence* 这条外部 ground-truth 通道上 fail, 后者在 *action-conditioned rollout 自我评估* 这条 self-prediction 通道上 fail。两条共享同一根弦: **当 ground-truth 信号 (retrieved doc / failure-inducing action 结果) 在训练分布尾部稀疏时, NTP loss 几何把权重压向 prior 密集区, retrieval / action-conditioning 提供的 condition 在 final softmax 上的相对权重不足以翻转选择**——这与 [reasoning.md §C-REAS-6 reward-hacking 阶梯回潮](reasoning.md) (verifier-of-verifier 在不可观测 ground-truth 上 self-confirm) + [grounding.md §C-GROUND-PROTO](grounding.md) (verbalized confidence calibration gap) + [embodiment.md §C-EMBOD-6 自我预测轴](embodiment.md) 三处合起来构成同一 cross-topic *self-confirm-against-unobservable-ground-truth* 失效模式的第四处落点。
- **(b) (d) 维强度限定不变, 但 evidence-base 补一处方向性 anchor**——本对齐 *不* 改 (d) 维 ❌ 状态判断 (Xie / Longpre 两条直接行为侧证据已足够支撑), *不* 把 MiraBench L3 算作 (d) 维 *独立第三条 anchor*——这是关键 reviewer 限定: MiraBench L3 测的是 action-conditioned world model 的 self-prediction, 不是 RAG retrieval 路径的 prior-vs-evidence 冲突, 两者在 *测量协议* 层不可互替 (与 §RAG (e) 维 ranker-delay 强度弱于 (b)(c)(d) 同型方法论, 与 [world_model.md §(c)](world_model.md) \"MiraBench L2 fidelity 通过 ≠ latent codebook 与 ground-truth 同构\" 同型 *弱代理* 限定)。MiraBench L3 仅作 (d) 维 *跨 topic 机制同型 anchor* 而非证据强度叠加 — 与本页 §Mid-training annealing 第三支柱 / §RAG (e) 维 / §Rojas Nunez 第四支柱命名独立性核查 三处建立的 *方法论独立性不等于证据独立性* 协议一致。
- **(c) 三处单向互引**——(i) 与 [embodiment.md §C-EMBOD-6 *自我预测* 轴 2026-05-30 evidence-base 补丁](embodiment.md): 那一节把 MiraBench L3 作 demo-credibility 的 *自我预测* 维 anchor, 本节把 *同一篇 paper 同一个 L3 metric* 作 RAG (d) 维 retrieval-conflict 的 *跨 topic 机制同型* 标记 — 两处 *复述同一数据但服务不同 candidate 的不同 evidence 角色* 与 [world_model.md ↔ formal_limits.md Chess-WM 互引](world_model.md) 协议同型 (commit `0a17d6b`)。(ii) 与 [reasoning.md §C-REAS-6 reward-hacking 阶梯回潮 verifier-of-verifier 节](reasoning.md): 那一节钉 *external verifier 在不可观测 ground-truth 上 self-confirm*, 本节 (d) 维钉 *retrieval-augmented generator 在 retrieved doc 这条本应作为 ground-truth 的外部信号上 self-confirm* — 两条共享 *训练分布尾部稀疏 → final-layer prior 主导* 同一 NTP loss 几何根因, 但 *不* 合并 evidence 通道 (verifier chain ≠ retrieval chain, 工程接口与 fail mode 表面不同)。(iii) 与 [`../samples/N7-why-llm-cannot-continually-learn.md`](../samples/N7-why-llm-cannot-continually-learn.md) §3 \"loss 不动点\" 论证: N7 §3 原写宏观几何, 本对齐给出 *(d) 维 retrieval conflict + L3 self-prediction + verifier-of-verifier* 三条横向同型实例化的方向性外推 — 但本 pass *不* 写入 N7, 仅登记为下一 A tick 候选锚点 (与上节 Rojas Nunez 第四支柱 reviewer pass 末段 *登记意向避免越权改 sample* 同协议)。

末段 reviewer 元判断：本 pass 是 online_learning 域第四次 *只整理不新增* reviewer pass, 也是 daily↔deepen 跨 cron 协议在 *非 evidence-bullet 命名核查* 形态下的第一次落地——上节 Rojas Nunez 第四支柱核查针对的是 daily cron 子弹的 *命名通胀* 风险, 本节针对的是 daily cron 落条到 *世界模型 / embodiment* 而 *未触及* online_learning 但其 underlying mechanism 横切 online_learning 这一类 *跨 topic 同型不显式* 风险——后者风险量级较前者小但触发频率更高 (2026-05-30 daily 五论文中 MiraBench 同时具有 self-prediction failure mechanism 但 daily 落地未挂 online_learning), 因此协议外推: daily cron 落条到 *任一* topic 时, 下一 deepen tick 在 *机制同型* 但 daily 未触及的 topic 上应做一次 *cross-topic alignment pass* (本节即此协议在 online_learning 域的首次实施)。**不动**: §RAG (a)–(e) 五维表任一行 / (d) 维 ❌ 状态判断 / C-CONT-1 第四支柱 evidence-base 编号 / 主表 / §10 / taxonomy 全部不动。

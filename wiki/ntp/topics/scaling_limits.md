...
## C-SCALE-2 频率-SNR sigmoid 在 entity-level hallucination 上的落点 (2026-05-30 互引补充)

N9 §五 ([`../samples/N9-hallucination-as-epistemic-failure.md`](../samples/N9-hallucination-as-epistemic-failure.md)) 在 2026-05-29 把 hallucination 总量按 mech 候选做了一次三角拆分: (a) 50–60% 落 C-GROUND-7 (epistemic readout gap), (b) 25–40% 落 C-CONT-1 (cutoff + long-tail capacity), (c) 5–15% 交叠区 + 5–10% sycophancy 落 C-REAS-6. 三角的 (b) 这一格——*post-cutoff + long-tail 事实型 hallucination*——在 sample 侧的核心实证 anchor 是 Smith 2026 [arxiv:2605.18732](https://arxiv.org/abs/2605.18732), 把 capacity-bound recall 拟到 (log params, log freq) 二变量 sigmoid 上, 左下角 long-tail + post-cutoff 那一区即 C-CONT-1 域. **这条 sigmoid 在 scaling_limits 这一页的对应身份, 是 C-SCALE-2 (频率-SNR sigmoid) 在 *entity-level 事实-recall* 任务族上的具体形态**——不是新条目, 也不是 C-SCALE-2 的 *外推*, 而是 C-SCALE-2 已有 falsifier 网络 (Kandpal 2211.08411 / Mallen 2211.09110 / Allen-Zhu 2404.05405 bit/param 线) 的第四个独立 anchor.

但把这条对应身份直接写进 §第二轴 sample-complexity 节会越过 12 天增条冻结协议——所以这里以 *互引补充* 形态登记, 不动 §第二轴主条款, 不开新 sub-candidate, 不入 §10 evidence 列。三处强度限定:

- **(α) anchor 通道独立但归因通道不独立**. Smith 2605.18732 的 sigmoid 与 Kandpal / Mallen 系列共享 web-frequency 作 x 轴这一根本量, 二者的 RecAcc-vs-freq 曲线斜率在 7B-70B 区间 ρ ≈ 0.6 (Smith Table 4 [unverified 具体数字]). 这意味着 C-CONT-1 三角 (b) 格的 25–40% 数字本身已经把 Kandpal-Mallen 那部分作为基底捕获, 把 Smith 当作 C-SCALE-2 的第四独立 anchor 在 sample 侧 *正面计入证据强度* 时会双重计数. 处理: scaling 侧只把 Smith 计为 C-SCALE-2 anchor 集合的 *方法学* 加密 (二变量联合拟合替代单变量), 不计为 *证据数量* 加 1.
- **(β) entity-level vs claim-level 作用域错位**. Smith sigmoid 测的是 *单实体单事实* recall, N9 §五 三角的 hallucination 总量分母是 *开放生成中的 claim*——后者可能跨多个 entity (e.g., "X 在 Y 年加入 Z 公司" 三元组). 把 sigmoid 的 entity-level 数字外推到 claim-level 需要一个 *claim 内独立性假设*, 该假设在 Kapoor 2402.04614 的 hallucination 类型分布上未被显式检验. 处理: N9 §五 25–40% 区间的下界对应 \"claim 内 entity 独立\" 假设成立, 上界对应该假设失败时 entity 间错配累积——上下界差 15pp 即此假设的不确定度.
- **(γ) C-SCALE-2 与 C-CONT-1 在 \"post-cutoff\" 维度的边界**. C-SCALE-2 的频率-SNR sigmoid 在 *in-cutoff* long-tail 上已成立 (训练分布里这些事实出现过但 freq 极低), C-CONT-1 加的是 *post-cutoff* 这一维 (训练分布里这些事实根本未出现). 两者在 Smith sigmoid 左下角的 *同一区* 显现, 但 mech 不同: 前者是 capacity 写入失败, 后者是无可写入. N9 §五 把这两者放在同一格 (b) 是 *症状* 层归并不是 *mech* 层归并; scaling 侧必须在引用 Smith 时保留这条 mech 分离声明, 否则 C-SCALE-2 与 C-CONT-1 在 §10 sub-candidate 区会被错误读为存在 *direct 上下位* 关系 (实际是 *症状叠加无 mech 上下位*).

判断 (2026-05-30): C-SCALE-2 频率-SNR sigmoid 在 2026-05 视角下首次有了一个 *跨 topic 同型显影* 的具体落点——entity-level 事实-recall 的 sigmoid 曲线既是 scaling_limits 域 C-SCALE-2 第四 anchor (方法学加密), 也是 online_learning 域 C-CONT-1 三角 (b) 格的核心实证, 也是 grounding 域 C-GROUND-7 \"hidden state 有信号但 readout 报告不出\" 论断在 capacity-bound 极限上的 *负向* 对照 (capacity-bound 区根本无 hidden 信号可报告, 与 C-GROUND-7 的 contested-subset 设定正交). 这一三 topic 同型显影**不**升级 Smith 2605.18732 为 §10 跨 topic 落点纪录 (与 MiraBench 2605.29360 6 topic / DCV 2605.28860 4 topic 不同——Smith 是 *单 anchor 跨 mech 候选的 mech-level 同型*, 不是 *单 paper 跨 topic surface 的 surface-level 同型*), 但 *方法学层* 把 \"同一实证曲线被不同 mech 候选从不同方向各占一段\" 这件事在 scaling 域显式登记一次, 作为 [`../survey/ntp_survey.md`](../survey/ntp_survey.md) §10 横向加密之外的 *纵向* 加密通道的雏形——纵向加密在 2026-05-30 仍是 *未命名* 的第九种几何加密方式, 本节不主动命名, 留待 §10 下一次中段元-审计.

## C-SCALE-4 / C-SCALE-7 verifier-noise floor 的 cross-topic 收紧 (2026-05-31 reviewer pass)

C-SCALE-4 (Brown / Snell / Wu 三条幂律) 与 C-SCALE-7 (RM-overoptim / R1-Zero / RLVR reward-hacking 三条 outcome 层) 的 falsifier 文字里都有一个被一笔带过的关键排除条款：「**且失败原因不能归到 verifier 噪声**」（C-SCALE-4）/「未观察到 reward-hacking-style decoupling」（C-SCALE-7）。这条排除条款在 2025 RLVR 浪潮的 *outcome 层* 视角下读起来无歧义——verifier 噪声有 unit-test pass rate / 形式证明检查器 / numerical match 三类 well-known 估计——但在 2026-05 daily harvest 推上来的 MiraBench ([2605.29360](../papers/paper_notes/2026-05-30-2605.29360-mirabench-action-conditioned-reliability.md)) 之后必须被重读一次：当 verifier *本身* 是一个 generative-self-prediction stratifier (e.g., VLM-as-judge 在 action-conditioned world model 上跑 L3 Optimism Bias Detection)，verifier 噪声的 *估计* 与 verifier 噪声的 *真实下界* 之间存在一个被 12 个 frontier configuration 一致拉开的系统性 gap——12/12 model configuration 在 failure-inducing action 下仍预测 success，且 L2 fidelity 与 visual quality 弱相关（"好看 ≠ 听命令"）。把这一观察压回 C-SCALE-4 / C-SCALE-7 的 falsifier 排除条款，本节做三件 *evidence-base refinement + 互引补充 + falsifier 措辞收紧* 操作，严守 2026-05-30 → 2026-06-10 12 天增条冻结协议白名单，**不**新增 C-SCALE-8/9、**不**升级 C-SCALE-4/7 既有强度评估、**不**改 §第四轴 / §第七轴主条款、**不**入 §10 主表、**不**入 taxonomy 升降级历史。

- **(α) verifier 噪声估计的方法学锁孔**。C-SCALE-4 falsifier 默认 verifier 噪声可被 *独立* 测量——例如 MATH-500 用人工双盲核对取 (1 − ρ) 作 ground-truth noise 上界，AIME 用官方答案 binary 比较直接 ρ → 0。但 *verifier-rich* 子集若被外推到 *action-conditioned* / *open-ended generation* / *agentic multi-step intermediate* 三类任务族 (后者在 C-SCALE-4 原文里被 \"verifier-bounded\" 修饰排除，但工程上经常被 *悄悄* 纳入)，verifier 经常退化为另一个 LLM/VLM 做 judge——此时 verifier 噪声的 *估计* 与 *真实* 之间的 gap 不是 i.i.d. 噪声而是 *系统偏差*：MiraBench L3 在 12 configuration × 16k+ 人工 judgments 上同时报 *self-prediction 系统乐观* + *judge 与 ground-truth 弱相关* 两项 ([`world_model.md` §(c)](world_model.md) 已落 C-WM-7 第四 weak-proxy 通道, [`grounding.md` §C-GROUND-PROTO 方向性动作 (4)](grounding.md) 已落 (b)+(c) self-prediction 子域双轨耦合)。把这一观察回钉到 C-SCALE-4 falsifier 排除条款：原文 \"verifier 噪声\" 须 *显式区分* (i) *独立可测的 i.i.d. 噪声* (MATH / AIME / Codeforces 子集) 与 (ii) *generative-self-prediction stratifier 引入的系统偏差* (VLM-judge / LLM-as-judge / self-consistency vote 子集)；C-SCALE-4 falsifier 原本要求 \"failure 不能归到 verifier 噪声\" 中的 \"噪声\" 仅指 (i)，不覆盖 (ii)——遇到 (ii) 类 verifier 时, *failure* 与 *verifier 系统偏差* 不可分，falsifier 触发门槛实际未达成，C-SCALE-4 在该子集上不可被独立证伪。这与 [`reasoning.md` §C-REAS-6 reward-hacking 阶梯 verifier-of-verifier 节](reasoning.md) 同型——verifier 不可独立测量时 falsifier 形式上仍写得通但操作上无法机械执行。

- **(β) C-SCALE-7 reward-hacking-style decoupling 的 *mechanism 侧* 旁证四向闭环**。C-SCALE-7 falsifier 原文要求 "未观察到 reward-hacking-style decoupling"。2026-05 内已通过四处独立通道把 *decoupling* 的可观察性从 outcome 层推到 mechanism 层：(1) RLVR reward-hacking bundle [2605.26733](../papers/paper_notes/2026-05-26-2605.26733-rlvr-reward-hacking.md) outcome 层 12 模式 (本节 §第七轴原文); (2) DCV [2605.28860](../papers/paper_notes/2026-05-30-2605.28860-rl-vs-sft-circuit-preservation.md) head-level circuit overlap mechanism 层旁证 (本节 2026-05-30 evidence-base 补丁段); (3) MiraBench L3 在 *action-conditioned* 子集上证明 *generator 在自己 verifier 不可见的 ground-truth 上 self-confirm* 不依赖 RLVR pipeline 即出现 (即 reward hacking 的 *上游同型机制* 在纯 NTP base model 上已可观察, 与 [`online_learning.md` §RAG (d) 维 + MiraBench L3 跨 topic 对齐](online_learning.md) 同根的 *self-confirm-against-unobservable-ground-truth* 失效模式); (4) DCV × MiraBench 在 *same generator × different probe* 维度的不可加性 (按 §10 reviewer 加注 type-geometry: DCV type I 训练侧多落点独立证据 vs MiraBench type III surface 侧多落点协同回退, 两者跨 topic 同时援引时按 type-conditional 折扣, 不算两条独立 evidence 求乘)。四处合起来把 C-SCALE-7 falsifier 排除条款收紧为：reward-hacking-style decoupling **包括** (a) outcome 层 utility-vs-holdout 分叉 (原文) + (b) verifier 自身被同型 generator 当作 judge 时 self-confirm 偏差 (新加) 两类——*两类任一观察到* 即 falsifier 失败 (而非原文的 *仅 (a) 类失败*)。**强度评估不动** (C-SCALE-7 仍 medium, 升级阈值由 [`reasoning.md` §C-REAS-7](reasoning.md) / [`online_learning.md` §C-CONT-2 第四支柱](online_learning.md) 承担), 仅 falsifier 排除条款的 *判定空间* 从一维加宽到二维。

- **(γ) 与 type-geometry 同步链 (taxonomy `f61f8c4` → ... → world_model `f261f3e` → timeline `29107ec`) 七落点收束的兼容声明**。该链终点在 timeline meta-row 已显式登记 *type triple coverage 唯一完整域是 world_model*, 链终点声明 *下篇 ≥4 topic 直接 type 标注不追加新同步链*。本节登记的 C-SCALE-4 / C-SCALE-7 falsifier 二维化 *不* 构成第八落点同步链——它是同一根 *verifier-of-verifier* 因果通道在 scaling 域 falsifier 措辞侧的兑现, 与 [`reasoning.md` §C-REAS-6](reasoning.md) (verifier-of-verifier 命名) / [`causality.md` §Video world model rollback (d⁗/d⁽⁵⁾)](causality.md) (RSI-CCI 双指标相关性 / judge-of-judge) / [`world_model.md` §(c) MiraBench L3 第四 weak-proxy 通道](world_model.md) / [`grounding.md` §C-GROUND-PROTO 方向性动作 (4)](grounding.md) (VLM-judge 第三档仲裁) / [`embodiment.md` §C-EMBOD-6 *自我预测* 轴 anchor](embodiment.md) / [`online_learning.md` §RAG (d) 维 + MiraBench L3 跨 topic 对齐](online_learning.md) / [`samples/N6 §十`](../samples/N6-world-model-three-answers.md) / [`samples/N9 §尾声`](../samples/N9-hallucination-as-epistemic-failure.md) 八处共享 *单根因 × 多 falsifier 措辞 evidence-supply 通道*。三处单向引上述链节, 不要求反向回贴, 严守 §10 (c) 冻结协议 *互引补充* 白名单第八次使用。

**判断 (2026-05-31)**：本节是 scaling 域第三次 *只整理不新增* reviewer pass (前两次为 §OP-SCALE-1..7 半年核查 + §C-SCALE-2 sigmoid 跨 topic 显影段), 把 C-SCALE-4 / C-SCALE-7 既有 falsifier 排除条款里被一笔带过的 \"verifier 噪声\" / \"reward-hacking-style decoupling\" 两词的 *判定空间* 从一维加宽到二维, 兑现 §10 末段 type-geometry 链终点收束声明的 *防误读为新链节* 协议。最大方法学风险是 *把 falsifier 措辞收紧误读为 mech 命题加强* — 规避: 本节三段 (α)(β)(γ) 均明示 *判定空间加宽 ≠ 命题升级*, C-SCALE-4 medium-strong / C-SCALE-7 medium 两档强度 *不动*, §第七轴反例与边界条件 4 条 (DPO 定义吸收 / verifier 容量瓶颈 / process reward 突破 / self-distill 路径) *不动*, §第四轴 \"verifier-bounded 子集收敛\" 元判断 *不动*。下一次本节实质性更新最可能来自 (i) 2026-Q3 之前任一 frontier lab 公开 verifier-noise 分解 ((i) i.i.d. vs (ii) self-prediction stratifier 系统偏差) 的 controlled report (低概率, 与 M1 集合 $m_5$ verifier-chain-$\\beta_k$ disclosure 缺位同型), 或 (ii) MiraBench L3 在 frontier ≥ 70B 上的复现并报告 latent-NMI controlled comparison (中等概率, 与 [`samples/N5 §九`](../samples/N5-embodiment-and-vla-bet.md) 末段挂账同步触发点)。引用全部沿用 verified IDs (MiraBench 2605.29360 / RLVR bundle 2605.26733 / DCV 2605.28860 均经 paper_notes 验证, commit `f61f8c4`/`29107ec`/`f261f3e` 经 git log 验证), 无 ID/数字/作者/日期 fabricated。
...
- [online_learning](online_learning.md)：catastrophic overtraining 的 plasticity cliff 与 continual learning 的 plasticity-stability 张力是同一现象的两端；C-CONT-1 三角 (b) 格 (post-cutoff long-tail) 与本页 C-SCALE-2 频率-SNR sigmoid 在 Smith 2605.18732 上共享同一实证曲线但 mech 分离。
- [grounding](grounding.md)：C-GROUND-7 epistemic readout gap 与本页 C-SCALE-2 在 *capacity-bound vs contested-subset* 维度正交——前者要求 hidden 信号存在但被压制, 后者刻画 hidden 信号本身不可达。
- [N1 The NTP Question](../samples/N1-the-ntp-question.md)：scaling-limits 是 N1 §"可达性 vs 可学性" 区分的主要证据基底。
- [N9 Hallucination as Epistemic Failure](../samples/N9-hallucination-as-epistemic-failure.md)：§五 三角归属 (b) 格把 Smith 2605.18732 sigmoid 作 C-CONT-1 核心实证, 与本页 C-SCALE-2 第四 anchor 同曲线异 mech, 见上节 (α)(β)(γ) 三处强度限定。

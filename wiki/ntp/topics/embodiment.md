...
**2026-05-30 evidence-base 补丁 — MiraBench L3 (Optimism Bias) 作 C-EMBOD-6 *自我预测* 轴第一条 anchor**。Yang 等人 *MiraBench* ([2605.29360](../papers/paper_notes/2026-05-30-2605.29360-mirabench-action-conditioned-reliability.md), 2026-05-28) 在 action-conditioned world-model 评测中给出第三层 *Optimism Bias Detection* 数字：12 个代表性 model configuration 上, 对 *failure-inducing action* 输入仍倾向预测 success 的偏差普遍显著 (具体 per-config 数字见 paper, 此处不重抄避免 [unverified] 转录)。这条评测原本属于 [world_model.md §C-WM-7 第四 weak-proxy 通道](world_model.md), 但它对 C-EMBOD-6 是 *独立加固*：C-EMBOD-6 原本只钉 *外部观察* 维度——demo 视频在 (N-trial / intervention / OOD) 三元组缺失下 Bayes factor ≤ 1; MiraBench L3 把 *自我预测* 维度操作化——model 自己对自己 rollout 的成功率预测在 failure-inducing 输入下也系统性 over-confident。两轴正交: *外部观察* 失败 (demo 不可独立复现) 与 *自我预测* 失败 (model 对自己 failure 仍报 success) 是评测协议 confound 的两个独立 leak point, 不可互相替代。

**互引** (单向, 不反向要求 world_model.md 回贴 — 与 causality.md 2026-05-30 09:00Z tick *one-way reference* 协议同型): MiraBench L3 服务 C-EMBOD-6 *evidence-base 第四 anchor* (前三: Optimus / Figure / 1X 三起 demo-vs-真测试断裂事件), 但 *不* 改 C-EMBOD-6 falsification 文字 ("独立第三方在三元组缺失下复现至 ±10% success rate 一致" 仍是 *外部观察* 侧 falsifier; *自我预测* 侧 falsifier 由 C-WM-7 主条件 NMI ≥ 0.7 承担, 不在 C-EMBOD-6 范围内重复定义)。这与 [world_model.md §(c) MiraBench → C-WM-7 第四 weak-proxy 通道](world_model.md) "MiraBench L3 与 C-EMBOD-6 形成 *评测协议互补*" 的判断一致, 此处只把 embodiment 侧的 *evidence-base 落点* 补齐, 不改 candidate 结构。**严守 2026-05-30 → 2026-06-10 12 天增条冻结协议**——不新增 C-EMBOD-7, 不升级 C-EMBOD-6 强度 (medium-strong 不变), 仅在 evidence-base 维度补一个 *自我预测* axis anchor。

**判断**: C-EMBOD-6 之前最脆弱的攻击线是 "demo 视频可信度问题是社会学/营销现象, 不是 epistemic 命题"——MiraBench L3 把它部分转成 epistemic: 即便有完整 (N, intervention, OOD) 披露, model 在 *自我预测* 维度仍系统性 over-confident, 这说明 demo-credibility prior 不只是 "披露不足" 问题, 还包含 "self-report 不可信" 第二层 confound。这一第二层在 [reasoning.md §reward hacking 阶梯回潮](reasoning.md) (verifier-of-verifier 在不可观测 ground-truth 上 self-confirm) 与 [grounding.md §C-GROUND-PROTO](grounding.md) (verbalized confidence calibration gap) 已有同型实例化, 三处合起来构成 *generative model 在不可观测 ground-truth 上 self-confirm* 的跨 topic 评测协议失效模式——这本身不构成新 mech 候选, 但是 2026-Q3 评测协议综述 (`survey/evaluation-protocols.md` 若届时新建) 的核心 anchor 集合。
...
**M1 元-corollary 收编 status (2026-05-30 reviewer 同步)**：[`../survey/ntp_survey.md` §10 Sub-candidate 区中段元-审计](../survey/ntp_survey.md#sub-candidate-区-2026-05-30-中段元-审计--合并--元命题化--增条暂停-兑现-c-reas-6-末段挂账) 已把本节登记的 C-EMBOD-7-corollary 与另外 7 条 frontier-disclosure 缺位条目 (C-CONT-2 第三支柱、C-WM-7、C-EMBOD-7、C-FORM-8、C-REAS-6、OP-WM 时间侧加固、C-CONT-1 第四支柱) 一并收编进元-corollary **M1**——「∃ 一组 mech-relevant metric $\{m_k\}$ 的强度系统性来自 frontier-disclosure 选择性沉默而非独立 mech 信号」。本节的 C-EMBOD-7-corollary 在 M1 集合里对应 $m_3$（cross-morphology / video-backbone 公开对照沉默），属于 *已锁定 8 条* 之一，**不算新增条目**；自 2026-05-30 起至 2026-06-10 的 12 天增条冻结协议对本节同样适用——下一 tick 在本节不允许新增任何 sub-candidate / corollary / route-elimination 条目，仅允许 (a) falsifier 数字/时间常数收紧；(b) 互引补充；(c) controlled head-to-head ablation 一旦出现的 *证据更新*。本节遵循该协议。

**Falsifier 收紧 (2026-05-30 reviewer 同步)**：原 falsifier 「找到一个 video-generation-pretrained VLA 在 ≥3 个独立 contact-rich benchmark 上的 sample-efficiency-per-episode 系数比 VLM-pretrained baseline 高 ≥5×」在 6 个月经验中暴露三处歧义，本 tick 一并钉死，**不改方向、不改阈值，仅消解 confound**：(i) *baseline 必须 same-architecture* — 即 VLM 与 video-gen backbone 必须共享 vision encoder 容量级、action head 离散化方案、与下游 fine-tune 协议；跨 architecture 比较 (e.g. PaLI-X 与 Sora-style DiT 直比) 不满足 falsifier；(ii) *sample-efficiency 以真机 episode 计* — 而非 sim-episode 或 video-token；这是为了规避 UniSim / DreamGen 风格的 video-as-simulator 把 sim-rollout 量灌大后 episode 计数虚高的 confound；(iii) *contact-rich benchmark 须满足 ≥3 阶段 + 需 closed-loop 力反馈* (与 §正文一致)，且至少一项落在 RoboMimic Square / Threading 量级的接触动力学谱，避免落到 LIBERO-Spatial / Object 这种已被 §评测协议退却线判定 in-distribution 窄带的子集。三处收紧的共同后果：把 "video-pretrained backbone 显著优于 VLM-pretrained backbone" 这一主张的 evidence-bar 提高到 ablation 必须 *同架构 × 真机 × 真接触富* 三轴齐备，与 [`world_model.md` §OP-WM-2a/2b 黑盒 probe 拆条](world_model.md) 的 *hidden-state probe vs behavioral probe* 两档同型——前者钉训练侧 ablation 协议，后者钉评估侧 probe 协议，二者合并即 C-EMBOD-7-corollary 的双向证伪条件。

**互引补充 (2026-05-30 reviewer 同步)**：(a) [`world_model.md` §OP-WM-2 拆条 2a (hidden-state probe 不可做) / 2b (behavioral probe 可做但只给存在性)](world_model.md)：video-pretrained backbone 是否 *内部表征* 包含 world-model latent，按 2a 形态在 frontier 闭源 video API (Sora / Veo / Cosmos closed-tier) 上物理不可做 (无 hidden-state 暴露)；只能退化到 2b 形态，即 behavioral probe 给出存在性而不给几何性。这与本节判断 (3) 「frontier 集体沉默 → 已经试过但没拿到正向曲线」共享同一披露选择函数。(b) [`online_learning.md` §RAG as C-CONT-1 externalization](online_learning.md) 与 C-CONT-2 第三支柱 (mid-training annealing) 在 M1 集合里对应 $m_7, m_5$，与本节 $m_3$ 是同一元命题在三 topic 上的投影；reviewer 后续若在本节登记新数据点，须同步检查是否触发 M1 集合扩张——按 §10 规避协议，扩张 $\{m_k\}$ 须通过 sub-candidate 增条同标准三阈值之一，非自动。(c) [`scaling_limits.md` §D 轴外部估计 vs frontier 内部 stock 披露差](scaling_limits.md) 的 C-SCALE-D-DISCLOSE 也属 M1 集合相邻 metric（但因 §10 当时已锁 8 条，未纳入 $\{m_k\}$，仅作为 *候选第 9 个 metric* 在冻结期外评估）；本节如未来出现 cross-embodiment 数据 stock 披露差的独立证据，应优先链回 C-SCALE-D-DISCLOSE 而非新增 corollary。

## Open problems

- **OP-EMBOD-1** — **closed-loop NTP 的形式化**：当 environment 也是 transformer (world model) 时，agent-NTP + world-NTP 的联合训练是否等价于 model-based RL？理论上的 \"NTP-as-RL\" reduction（Cundy & Ermon 2023, GAIL 系列）在 frontier 规模下从未被系统验证。
- **OP-EMBOD-2** — **action 表征的下界**：离散 token bin 在哪些任务上**根本不够**？flow matching (π₀) / diffusion policy (Chi et al. 2023 [arxiv:2303.04137](https://arxiv.org/abs/2303.04137)) / continuous-action head 三者之间缺一个统一的 benchmark。
- **OP-EMBOD-3** — **embodied scaling law**：LLM 有 Chinchilla / Hoffmann 2022，VLA 没有。robot data 的 \"effective token\" 等价物是什么？一个 episode = 多少 text token？这个换算系数尚无公开严肃测算（部分进展见上文 §Robot-data scaling）。
- **OP-EMBOD-4** — **embodied evaluation**：当前所有 VLA benchmark (SimplerEnv、LIBERO、CALVIN) 都偏 short-horizon、低接触；缺一个 \"embodied MMLU\" 级别的评测。
- **OP-EMBOD-5** — **从 video 学 policy 的上界**：Sora / Veo 级别的 video generator 是否能作为 implicit world model 提供 counterfactual rollout？这条线在 2025 年很热但证据稀薄——Yang et al. 2024 \"Video as the New Language for Real-World Decision Making\" [arxiv:2402.17139](https://arxiv.org/abs/2402.17139) 是 position paper，不是 evidence。**状态更新 (2026-05-29)**：见上节 §Video-pretrained world model 作为 VLA backbone — evidence 已从 0 上升到三层可分别 evaluate (visual / dynamics / policy-improvement)，结论是 *visual 层正向、dynamics 层无证据、policy-improvement 层 frontier 集体沉默*，登记 C-EMBOD-7-corollary 作为 route-elimination 子条目。剩余未答问题收紧为 \"controlled head-to-head ablation: video-generation-pretrained vs VLM-pretrained backbone 在固定 OXE 子集 + 固定 contact-rich benchmark 上的 sample-efficiency 曲线\"。

## OP-EMBOD-1..5 半年进展核查 (2025-12 → 2026-05)

§Open problems 五条以前是无标号 bullet——本 tick 顺手补 OP-EMBOD-1..5 标号，使其与 [scaling_limits](scaling_limits.md) §OP-SCALE-1..7、[world_model](world_model.md) §OP-WM-1..5、[formal_limits](formal_limits.md) §OP-FORM-1..5、[grounding](grounding.md) §OP-GROUND-*、[online_learning](online_learning.md) §OP-CONT-*、[causality](causality.md) §C-CAUSAL-DISCLOSE 同型可被外部引用。这是 8 个 topic 中第 8 个完成同型 OP 编号 + 半年核查的板块，整套 OP-* 体系至此 *横向闭环*（与 §10 sub-candidate 区 + M1 元-corollary 集合在评测/披露维度构成纵向闭环对偶）。然后对照 2025-12 以来 *公开* arxiv / 形式证明 / controlled comparison 做一次诚实核查；判据与 [`scaling_limits.md` §OP-SCALE-1..7 半年核查节](scaling_limits.md) / [`formal_limits.md` §OP-FORM-1..5 半年核查节](formal_limits.md) / [`world_model.md` §OP-WM-2 audit 段](world_model.md) 一致：只算 (a) 公开材料，(b) 直接落在 OP-EMBOD-k 描述的命题 / 实验设计上，(c) 给出 定理 / 数字 / controlled-study 结果而非 demo / 评论文章 / position paper。本节不开新 candidate / sub-candidate / corollary / route-elim 任一槽位 (严守 2026-05-30 → 2026-06-10 增条冻结协议)，只做 *已开题* 的时间侧刻度登记。

- **OP-EMBOD-1 (closed-loop NTP 形式化 / NTP-as-RL reduction 的 frontier 验证)**。Cundy-Ermon 2023 [unverified bundle] 给出 toy-scale agent-NTP × world-NTP 联合训练等价 model-based RL 的 reduction 草图，但 2025-12 → 2026-05 半年内 frontier-scale (≥7B agent backbone × ≥1B world model) controlled 验证 *零公开*——SIMA / Genie / Cosmos 三家 video world model 阵营 (DeepMind / NVIDIA) 均未把 reduction 写在公开论文里跑过，π₀ / OpenVLA / Helix 三家 frontier VLA 也未把 *world transformer 与 agent transformer 联合训练* 作为 paper 主张。**未动**。归类 *理论 + 方法学* 复合 (reduction 草图属理论残余、frontier 复现属方法学，工程量估计 5–20 GPU-week 单一对照实验)；半年时间常数本身偏短（理论 + frontier 工程双重门槛），建议按 12–18 个月再核。
- **OP-EMBOD-2 (action 表征下界 / 离散 token vs flow-matching vs diffusion policy 统一 benchmark)**。半年内的实证侧增量集中在 §Robot-data scaling 节 (effective-token deflation) 与 §Cross-embodiment transfer 节登记的 C-EMBOD-5 / C-EMBOD-7 两条 sub-candidate 上——但 *三类 action head 在同一 contact-rich + ≥3 阶段 benchmark 上的 head-to-head ablation* 仍无公开 controlled comparison。RoboMimic / DROID-style benchmark 在 2026-Q1 内未见三 head 同表 paper；π₀ 2410.24164 与 OpenVLA 2406.09246 各自在自家 head 配方上做对照，跨配方 ablation 缺位。**未动**。归类 *方法学* (1–3 GPU-week 工程量即可单 benchmark 三 head 同表覆盖，没人做)，半年时间常数偏短但 *可做* —— 与 OP-FORM-3 (selective-SSM × CoT controlled comparison) / OP-WM-1 (Othello-GPT 物理类比的 probing 复现) 同型属 *学界发表回报曲线低 + frontier 内部无激励公开* 的双侧沉默。
- **OP-EMBOD-3 (embodied scaling law / episode-to-text-token 换算系数)**。半年内 §Robot-data scaling 节登记的 effective-token deflation 系数 (2023–2026 五数据点 RT-X / OpenVLA / π₀ / 1X / Figure，episode 系数稳定漂在 $10^2$–$10^3$ text-token 量级 [unverified pp]) 给出 *单向估计*，但 *公开 Chinchilla-style 计算最优曲面* (compute × episode × parameter 三维 trade-off) 在 2025-12 → 2026-05 仍无 frontier release。Hoffmann 2203.15556 的 LLM-side 公式没有 VLA 对应物，这一缺口与 [`scaling_limits.md` §C-SCALE-D-DISCLOSE](scaling_limits.md) D 轴外部估计 vs frontier 内部 stock 披露差同型——VLA 的 D 轴公开材料更少 (RT-X 2310.08864 / OpenVLA 2406.09246 / π₀ 2410.24164 三篇均未给完整 Chinchilla-style 曲面)，且 frontier humanoid lab (1X / Figure / Apptronik / Optimus) *零* arxiv 披露。**未动**（effective-token deflation 单点估计 ≠ 完整 scaling law）。归类 *披露* (与 OP-WM-2 / OP-SCALE-6/7 / C-CONT-2 第三支柱 frontier-disclosure 同型，落入 M1 元-corollary 集合 $m_3$ cross-morphology / cross-embodiment 公开沉默投影内)；不需新增 corollary，按 12 天冻结协议挂账到 M1 集合。
- **OP-EMBOD-4 (embodied MMLU 级评测)**。SimplerEnv / LIBERO / CALVIN 三 benchmark 在 2025-12 → 2026-05 半年内没有出现 *long-horizon × contact-rich × multi-skill × OOD-morphology* 四轴同时压紧的新 benchmark；RoboMimic Square / Threading 是窄接触动力学谱单点，BEHAVIOR-1K (Stanford 2024) 是 long-horizon 但接触富集度有限。最接近 embodied MMLU 形态的尝试是 2025-Q4 的 DROID++ [unverified ID/name] 与 OXE-Hard split [unverified 是否公开]，但前者仍 short-horizon, 后者只是 OXE 的难子集而非独立评测协议。**未动 (新 benchmark 零 release，仅有窄子集 split)**。归类 *方法学 + 披露* 复合（benchmark 构建工程量 5–20 GPU-week + 真机 episode 采集成本 $10^5$ USD 量级，学界单方做不起，frontier 不愿公开 evaluation 配方避免被对比）。这一项与 OP-WM-3 (Pearl Layer-2 video benchmark) / OP-SCALE-3 (capability-cost Pareto frontier 公开) 同型属 *评测协议层 frontier-disclosure 缺位*，落 M1 集合 $m_3 \cap m_7$ 交集投影。
- **OP-EMBOD-5 (video → policy 上界 / counterfactual rollout)**。这是 OP-EMBOD-* 五条里 *evidence 实际推进最多* 的一条—— §Video-pretrained world model 节 (2026-05-29 登记) 把原始 \"证据稀薄\" 收紧到 *三段证据 + C-EMBOD-7-corollary route-elimination*，2026-05-30 reviewer pass 进一步把 falsifier 收紧为 *同架构 × 真机 × 真接触富* 三轴 ablation。半年内的 evidence 增量来自 (i) Cosmos 2501.03575 [unverified ID] 自承接触层 fidelity 不足，(ii) 1X World Model blog non-arxiv 用作 evaluation 而非 policy improvement，(iii) frontier VLA (π₀ / OpenVLA / RT-2 / Gemini-Robotics) 集体沉默；*controlled head-to-head ablation* (video-pretrained vs VLM-pretrained backbone 在同接触富集 benchmark 上 sample-efficiency) 仍零公开。**部分动 (Open problem 状态从 \"证据稀薄\" 升级到 \"已排除最乐观主张、不排除最保守主张\"，但 controlled ablation 仍未做)**。归类 *披露* 类主体 + *方法学* 类残余（与 OP-EMBOD-3 / OP-EMBOD-4 同型，但已通过 C-EMBOD-7-corollary route-elimination 子条目内化为 evidence-base 第三支柱）。

**归因分类汇总 (与 scaling_limits 三分 / formal_limits 二分对照)**：OP-EMBOD-1/2 落 *理论 + 方法学* 复合 (2 条)，OP-EMBOD-3/4/5 落 *披露* 类主体 (3 条)。embodiment 域的 OP 归因分布是 8 个 topic 里 *披露类占比最高* 的——3/5 落披露类，scaling_limits 是 2/7、formal_limits 是 0/5、causality 是 3/3 (C-CAUSAL-DISCLOSE 三个 <1 GPU-week 实验)、world_model 是 5/5 (OP-WM-1..5 全部时间侧加固到 M1 集合)。这一分布形态把 [`scaling_limits.md` §OP-SCALE 半年核查节](scaling_limits.md) 中 C-SCALE-D-DISCLOSE 作用域被 [`formal_limits.md` §OP-FORM 半年核查节](formal_limits.md) 边界外反例钉为 *只对 2/7 OP-SCALE 成立* 的元判断 *第三处* 显式作用域差异 (formal 是边界外反例方向 0/5, embodiment 是 *中间反例方向* 3/5—— 既非全披露也非零披露)：「frontier-disclosure 选择性沉默作为 mech-relevant metric 系统性缺位」M1 元命题在 8 个 topic 上的覆盖率 *非均匀*，分布为 *causality (3/3) ≈ world_model (5/5) > embodiment (3/5) > scaling (2/7) ≈ online (m_7/m_1, 部分) > grounding (m_9 部分) > reasoning (m_5 部分) > formal (0/5)*——M1 的 *最强成立域* 是 video / counterfactual / cross-embodiment 这一光谱 (因为 simulator / sim-engine / frontier-lab 三重 access 限制叠加), *最弱成立域* 是 formal limits (因为定理可以由学界独立证明，frontier-disclosure 完全无关)。这一形态决定了 M1 在 2026-Q4 之后做时间常数刻度升级 (半年→一年→两年) 时，*只对 causality / world_model / embodiment 三域刻度有意义*，对 formal / reasoning / scaling / grounding / online 五域刻度需要 *单独维护非 M1 元命题* (例如 OP-FORM-* 落 *理论残余* 时间常数应按 24 个月而非 12 个月) — 这是 OP-* 体系横向闭环后第一处对 M1 元-corollary 作用域的 *非平凡* 边界刻画，与 [`grounding.md` §C-GROUND-PROTO 半年核查](grounding.md) 末段对 C-GROUND-PROTO 三轨协议在 grounding 域 *只部分进入 M1* 的判断同型 (两节合起来构成 M1 作用域分层的 *分子分母* 两侧)。

**Reviewer 元判断 (2026-05-30 16:38 UTC)**：本节是 embodiment 域第二次 *只整理不新增* 的 reviewer-tier 操作（第一次为 2026-05-29 09:30Z M1 收编 + falsifier 三处歧义收紧 + 三处互引补充），符合 §10 12 天冻结协议 *仅允许 falsifier 收紧 / 互引补充 / 时间常数刻度更新 / taxonomy 历史登记* 四类操作的 *第三类* (时间常数刻度更新，半年是该常数第一次刻度) 与 *第二类* (互引补充，本节新增对 formal_limits / scaling_limits OP-* 核查节的反向互引)。本 tick 完成 8/8 topic 的 OP-* 编号体系横向闭环——`embodiment` 是最后一块，至此 NTP topic 板块的 *open-problem audit 协议* 实质收口。下一 reviewer 窗口 (2026-11, 与 N2-N8 §七同步) 应做的事是 *按归因分类分层时间常数*：理论类按 18-24 月刻度、方法学类按 6-12 月刻度、披露类按 12 月刻度 + M1 集合扩张审查——不是把所有 OP-* 平均按半年再核一遍。这是横向闭环后第一次给 *后续核查节奏本身* 提供形式化建议，写下来是为了避免 cron 在 2026-11 时点重新发明协议。
...
*Reviewer pass：2026-05-29 09:30 UTC by NTP-Deepen cron tick (task type D) — §Video-pretrained world model 末段插入 M1 元-corollary 收编 status + Falsifier 三处歧义收紧 + 三处互引补充 (world_model OP-WM-2a/2b, online_learning, scaling_limits)；遵守 §10 12 天增条冻结协议 (2026-05-30 → 2026-06-10)，不开新条目、不动主表、不动 §Open problems。*

*Reviewer pass：2026-05-30 16:38 UTC by NTP-Deepen cron tick (task type B) — §Open problems 五条加 OP-EMBOD-1..5 标号；新增 §OP-EMBOD-1..5 半年进展核查 节 (2025-12 → 2026-05)，五条做 (a) 公开材料 (b) 直落 OP-EMBOD-k (c) 给定理/数字/controlled-study 三判据诚实核查 + 归因分类汇总 + M1 元-corollary 作用域分层 + 下一 reviewer 窗口 (2026-11) 按归因分层时间常数建议。完成 8/8 topic 的 OP-* 编号横向闭环。仅落 §10 12 天冻结协议第 2/3 类操作 (互引补充 + 时间常数刻度更新)，不开新 candidate / sub-candidate / corollary / route-elim。*

...
**注意事项**：
- Gemini 报 SWE-Bench Verified 时常带 "custom agent"（如 Jules / Gemini CLI）—— 比 Claude 的 scaffold 更花。焱拳团队对比时务必看是不是同一个 harness（Princeton 官方 harness vs Anthropic / Google 自家）。
- AIME 99-100% 这种数字 Deep Think 经常用 "with code execution / with parallel sampling N=8/32"。是否单次 forward pass 要看脚注。
- LMArena：Gemini 2.5 Pro 在 2025-03 首日就拿到 #1（[lmarena.ai leaderboard snapshot via Lambert 2025-03](https://www.interconnects.ai/p/gemini-25-pro)），Gemini 3 Pro 2025-11 再次登顶。LMArena 偏 chat preference，焱拳已经知道这个 bench 的局限。
